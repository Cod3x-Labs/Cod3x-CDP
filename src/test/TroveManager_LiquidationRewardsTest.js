  ...
  async (accounts) => {
    ...
    */
      ...
      const entireSystemColl = (
        ...
        .add(await defaultPool.getCollateral(collaterals[0].address))
        .toString();
      assert.equal(
        entireSystemColl,
    ...
    */
    ...
    it("redistribution: A, B Open. Protocol upgraded. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
      ...
      // C, D open troves
      const { collateral: C_coll } = await th.openTrove(newContracts, {
        collateral: collaterals[0],
        ICR: toBN(dec(440, 16)),
        extraParams: { from: carol },
      });
      const { collateral: D_coll } = await th.openTrove(newContracts, {
        collateral: collaterals[0],
        ICR: toBN(dec(230, 16)),
        extraParams: { from: dennis },
      });
      ...
      // Price drops to 100 $/E
      await newContracts.priceFeedTestnet.setPrice(
        collaterals[0].address,
        dec(100, 18),
      );
      ...
      // Confirm not in Recovery Mode
      assert.isFalse(
        await th.checkRecoveryMode(newContracts, collaterals[0].address),
      );

      // L2: D Liquidated
      const txD = await newContracts.troveManager.liquidate(
        dennis,
        collaterals[0].address,
      );
      assert.isTrue(txB.receipt.status);
      assert.isFalse(
        await newContracts.sortedTroves.contains(
          collaterals[0].address,
          dennis,
        ),
      );
    ...
    it("redistribution: A, B, C Open. C Liquidated. D, E, F Open. F Liquidated. Distributes correct rewards", async () => {
      ...
      assert.isFalse(
        await sortedTroves.contains(collaterals[0].address, freddy),
      );
    ...
    */
    ...
    it("redistribution: Sequence of alternate opening/liquidation: final surviving trove has ETH from all previously liquidated troves", async () => {
      ...
      const freddy_rawColl = (
        await troveManager.Troves(freddy, collaterals[0].address)
      )[1].toString();
    ...
    */
    ...
    it("redistribution: A,B,C,D,E open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
      ...
      const B_entireColl_1 = (
        await th.getEntireCollAndDebt(contracts, B, collaterals[0].address)
      ).entireColl;
      const C_entireColl_1 = (
        await th.getEntireCollAndDebt(contracts, C, collaterals[0].address)
      ).entireColl;
      const D_entireColl_1 = (
        await th.getEntireCollAndDebt(contracts, D, collaterals[0].address)
      ).entireColl;
      const E_entireColl_1 = (
        await th.getEntireCollAndDebt(contracts, E, collaterals[0].address)
      ).entireColl;
    ...
    it("redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
      ...
      const A_entireColl_0 = (
        await th.getEntireCollAndDebt(contracts, A, collaterals[0].address)
      ).entireColl;
      ...
      const denominatorColl_1 = (
        await troveManager.getEntireSystemColl(collaterals[0].address)
      ).sub(A_entireColl_0);
      ...
      const B_ETHGain_1 = await troveManager.getPendingCollateralReward(
        B,
        collaterals[0].address,
      );
      const C_ETHGain_1 = await troveManager.getPendingCollateralReward(
        C,
        collaterals[0].address,
      );
      const D_ETHGain_1 = await troveManager.getPendingCollateralReward(
        D,
        collaterals[0].address,
      );
      const E_ETHGain_1 = await troveManager.getPendingCollateralReward(
        E,
        collaterals[0].address,
      );
      ...
      const B_ETHGain_2 = await troveManager.getPendingCollateralReward(
        B,
        collaterals[0].address,
      );
      const D_ETHGain_2 = await troveManager.getPendingCollateralReward(
        D,
        collaterals[0].address,
      );
      const E_ETHGain_2 = await troveManager.getPendingCollateralReward(
        E,
        collaterals[0].address,
      );
      ...
      const B_ETHGain_3 = await troveManager.getPendingCollateralReward(
        B,
        collaterals[0].address,
      );
      const D_ETHGain_3 = await troveManager.getPendingCollateralReward(
        D,
        collaterals[0].address,
      );
    ...
    it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
      ...
      assert.isTrue(txC.receipt.status);
      assert.isFalse(
        await sortedTroves.contains(collaterals[1].address, carol),
      );
      ...
      assert.isTrue(txA.receipt.status);
      assert.isFalse(
        await sortedTroves.contains(collaterals[1].address, alice),
      );
      ...
      const bob_Coll = (
        await troveManager.Troves(bob, collaterals[1].address)
      )[1]
        .add(
          await troveManager.getPendingCollateralReward(
            bob,
    ...
    */
      ...
      assert.isAtMost(
        th.getDifference(alice_LUSDDebt, expected_A_debt),
        10 ** 9,
      );
    ...
    it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
      ...
      const alice_ETHReward_1 = await troveManager.getPendingCollateralReward(
        alice,
        collaterals[0].address,
      );
      const bob_ETHReward_1 = await troveManager.getPendingCollateralReward(
        bob,
        collaterals[0].address,
      );
      const carol_ETHReward_1 = await troveManager.getPendingCollateralReward(
        carol,
        collaterals[0].address,
      );
    ...
    */
      ...
      const entireSystemDebt = (
        await activePool.getLUSDDebt(collaterals[0].address)
      ).add(await defaultPool.getLUSDDebt(collaterals[0].address));
      th.assertIsApproximatelyEqual(
        entireSystemDebt,
    ...
    */
      const { collateral: A_coll } = await openTrove({
        collateral: collaterals[0],
        value: toBN(dec(450, collDecimals)),
        ICR: toBN(dec(90000, 16)),
        extraParams: { from: alice },
      });
      const { collateral: B_coll } = await openTrove({
        collateral: collaterals[0],
        value: toBN(dec(8901, collDecimals)),
        ICR: toBN(dec(1800000, 16)),
        extraParams: { from: bob },
      });
      const { collateral: C_coll } = await openTrove({
        collateral: collaterals[0],
        value: toBN(dec(23902, collDecimals.sub(toBN(3)))),
      ...
      const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({
        collateral: collaterals[0],
        value: toBN(dec(35, collDecimals.sub(toBN(3)))),
        extraLUSDAmount: dec(100, 18),
        extraParams: { from: dennis },
      });
    ...
    */
      const { collateral: E_coll, totalDebt: E_totalDebt } = await openTrove({
        collateral: collaterals[0],
        value: toBN(dec(10000, collDecimals)),
        extraLUSDAmount: dec(100, 18),
        extraParams: { from: erin },
      });
      const { collateral: F_coll, totalDebt: F_totalDebt } = await openTrove({
        collateral: collaterals[0],
        value: toBN(dec(7, collDecimals.sub(toBN(4)))),
        extraLUSDAmount: dec(100, 18),
        extraParams: { from: freddy },
      });
