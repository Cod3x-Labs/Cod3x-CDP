...
contract("TroveManager - in Recovery Mode", async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    coll0Decimals = await contracts.collateralConfig.getCollateralDecimals(
      collaterals[0].address,
    );
    coll0MCR = await contracts.collateralConfig.getCollateralMCR(
      collaterals[0].address,
    );
  ...
  it("checkRecoveryMode(): Returns true if TCR falls below CCR", async () => {
    ...
    const recoveryMode_After = await th.checkRecoveryMode(
      contracts,
      collaterals[0].address,
    );
  ...
  it("checkRecoveryMode(): Returns true if TCR stays less than CCR", async () => {
    ...
    const TCR_1 = (
      await th.getTCR(contracts, collaterals[1].address)
    ).toString();
    ...
    const recoveryMode_Before_1 = await th.checkRecoveryMode(
      contracts,
    ...
    assert.isTrue(recoveryMode_After);
    const recoveryMode_After_1 = await th.checkRecoveryMode(
      contracts,
      collaterals[1].address,
    );
  ...
  it("liquidate(), with ICR < 100%: removes stake and updates totalStakes", async () => {
    ...
    const recoveryMode = await th.checkRecoveryMode(
      contracts,
      collaterals[0].address,
    );
  ...
  it("liquidate(), with ICR < 100%: closes the Trove and removes it from the Trove array", async () => {
    ...
    const bob_TroveStatus_Before = (
      await troveManager.Troves(bob, collaterals[0].address)
    )[3];
    const bob_Trove_isInSortedList_Before = await sortedTroves.contains(
      collaterals[0].address,
      bob,
    );
    ...
    const bob_TroveStatus_After = (
      await troveManager.Troves(bob, collaterals[0].address)
    )[3];
    const bob_Trove_isInSortedList_After = await sortedTroves.contains(
      collaterals[0].address,
      bob,
    );
  ...
  it("liquidate(), with 100 < ICR < MCR: removes stake and updates totalStakes", async () => {
    ...
    assert.isAtMost(
      th.getDifference(
        TCR,
        scaledColl.mul(price).div(A_totalDebt.add(B_totalDebt)),
      ),
      1000,
    );
   ...
   */
    ...
    */
    const L_LUSDDebt = (
      await troveManager.L_LUSDDebt(collaterals[0].address)
    ).toString();
    const L_ETH = (
      await troveManager.L_Collateral(collaterals[0].address)
    ).toString();

    assert.isAtMost(
      th.getDifference(
        L_LUSDDebt,
        B_totalDebt.sub(spDeposit)
          .mul(toBN(10).pow(coll0Decimals))
  ...
  it("liquidate(), with ICR > MCR, trove has lowest ICR, and StabilityPool is empty: does nothing", async () => {
    ...
    await assertRevert(
      troveManager.liquidate(bob, collaterals[0].address, { from: owner }),
    ...
    const bob_isInSortedTrovesList = await sortedTroves.contains(
      collaterals[0].address,
      bob,
    );
    ...
    */
    ...
    assert.isAtMost(
      th.getDifference(
        ...
        th.applyLiquidationFee(
          th.unwrapCollNativeDec(scaledColl, coll0Decimals),
        ),
      ),
      3000,
    );
    ...
    let bob_remainingCollateral = B_coll.sub(
      th.unwrapCollNativeDec(
        B_totalDebt.mul(th.toBN(dec(12, 17))).div(price),
        coll0Decimals,
      ),
    );
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getUserCollateral(bob, collaterals[0].address),
  ...
  it("liquidate(), with  MCR < ICR < TCR, and StabilityPool LUSD > debt to liquidate: removes stake and updates totalStakes", async () => {
    ...
    assert.isTrue(
      bob_ICR.gt(mv._MCR) &&
        bob_ICR.lt(await th.getTCR(contracts, collaterals[0].address)),
    );
    ...
    const bob_remainingCollateral = B_coll.sub(
      th.unwrapCollNativeDec(
        B_totalDebt.mul(coll0MCR).div(price),
        coll0Decimals,
      ),
    );
    th.assertIsApproximatelyEqual(
  ...
  it("liquidate(), with MCR < ICR < TCR, and StabilityPool LUSD > debt to liquidate: can liquidate troves out of order", async () => {
    ...
    const ICR_A = await troveManager.getCurrentICR(
      alice,
      collaterals[0].address,
      price,
    );
    const ICR_B = await troveManager.getCurrentICR(
      bob,
      collaterals[0].address,
      price,
    );
    ...
    const ICR_D = await troveManager.getCurrentICR(
      dennis,
      collaterals[0].address,
      price,
    );
    ...
    const liquidationTx_D = await troveManager.liquidate(
      dennis,
      collaterals[0].address,
    );
    ...
    const liquidationTx_B = await troveManager.liquidate(
      bob,
      collaterals[0].address,
    );
    ...
    const dennis_remainingCollateral = D_coll.sub(
      th.unwrapCollNativeDec(dennis_scaledCollateral, coll0Decimals),
    );
    const bob_remainingCollateral = B_coll.sub(
      th.unwrapCollNativeDec(bob_scaledCollateral, coll0Decimals),
    );
    const carol_remainingCollateral = C_coll.sub(
      th.unwrapCollNativeDec(carol_scaledCollateral, coll0Decimals),
    );
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getUserCollateral(dennis, collaterals[0].address),
    ...
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getUserCollateral(carol, collaterals[0].address),
      carol_remainingCollateral,
    );

    // can claim collateral
    const dennis_balanceBefore = th.toBN(
  ...
  it("liquidate(), with ICR > MCR, and StabilityPool LUSD < liquidated debt: Trove remains in TroveOwners array", async () => {
    ...
    const arrayLength = (
      await troveManager.getTroveOwnersCount(collaterals[0].address)
    ).toNumber();
    ...
    for (let i = 0; i < arrayLength; i++) {
      const address = (
        await troveManager.TroveOwners(collaterals[0].address, i)
      ).toString();
      if (address == bob) {
        addressFound = true;
        addressIdx = i;
  ...
  it("liquidate(), with ICR > MCR, and StabilityPool LUSD < liquidated debt: ICR of non liquidated trove does not change", async () => {
    ...
    assert.equal(
      (await sortedTroves.getLast(collaterals[0].address)).toString(),
      bob,
    );
    assert.isTrue(
    ...
    const carol_Coll_Before = (
      await troveManager.Troves(carol, collaterals[0].address)
    )[1];
    ...
    await assertRevert(
      troveManager.liquidate(carol, collaterals[0].address),
      "TroveManager: nothing to liquidate",
    );
  ...
  it("liquidate() with ICR > MCR, and StabilityPool LUSD < liquidated debt: total liquidated coll and debt is correct", async () => {
    ...
    assert.isTrue(
      ICR_A.gt(coll0MCR) &&
        ICR_A.lt(await th.getTCR(contracts, collaterals[0].address)),
    );
  ...
  it("liquidate(): Doesn't liquidate undercollateralized trove if it is the only trove in the system", async () => {
    ...
    await assertRevert(
      troveManager.liquidate(alice, collaterals[0].address, { from: owner }),
      "TroveManager: nothing to liquidate",
    );
  ...
  it("liquidate(): does nothing if trove ICR >= TCR, and SP covers trove's debt", async () => {
    ...
    // Try to liquidate A
    await assertRevert(
      troveManager.liquidate(A, collaterals[0].address),
      "TroveManager: nothing to liquidate",
    );
    ...
    assert.equal(
      await troveManager.getTroveStatus(A, collaterals[0].address),
      1,
    ); // Status 1 -> active
    ...
    assert.equal(
      await troveManager.getTroveStatus(C, collaterals[0].address),
      3,
    ); // Status liquidated
  });
  ...
  it("liquidate(), with MCR < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
    ...
    const bob_surplus = B_coll_2.sub(
      th.unwrapCollNativeDec(redeemedCollateralScaled, coll0Decimals),
    );
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getUserCollateral(bob, collaterals[0].address),
      bob_surplus,
  ...
  it("liquidateTroves(): With all ICRs > MCR, Liquidates Troves until system leaves recovery mode", async () => {
    await contracts.collateralConfig.updateCollateralRatios(
      collaterals[0].address,
      toBN(dec(11, 17)),
      toBN(dec(15, 17)),
    );
    ...
    */
    ...
    const greta_ICR = await troveManager.getCurrentICR(
      greta,
      collaterals[0].address,
      price,
    );
    const harry_ICR = await troveManager.getCurrentICR(
      harry,
      collaterals[0].address,
      price,
    );
    ...
    // get all Troves
    const alice_Trove = await troveManager.Troves(
      alice,
      collaterals[0].address,
    );
    const bob_Trove = await troveManager.Troves(bob, collaterals[0].address);
    const carol_Trove = await troveManager.Troves(
      carol,
      collaterals[0].address,
    );
    const dennis_Trove = await troveManager.Troves(
      dennis,
      collaterals[0].address,
    );
    const erin_Trove = await troveManager.Troves(erin, collaterals[0].address);
    const freddy_Trove = await troveManager.Troves(
      freddy,
      collaterals[0].address,
    );
    const greta_Trove = await troveManager.Troves(
      greta,
      collaterals[0].address,
    );
    const harry_Trove = await troveManager.Troves(
      harry,
      collaterals[0].address,
    );
  ...
  it("liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves", async () => {
    ...
    assert.isTrue(
      (
        await troveManager.getCurrentICR(freddy, collaterals[0].address, price)
      ).lte(coll0MCR),
    );
    ...
    assert.equal(
      (await troveManager.Troves(freddy, collaterals[0].address))[3].toString(),
      "3",
    );
  });
  ...
  it("liquidateTroves(): A liquidation sequence of pure redistributions decreases the TCR, due to gas compensation, but up to 0.5%", async () => {
    ...
    assert.isAtMost(
      th.getDifference(
        TCR_Before,
        th
          .normalizeColl18Dedc(totalCollBefore, coll0Decimals)
          .mul(price)
          .div(totalDebtBefore),
      ),
      1000,
    ...
    assert.isAtMost(
      th.getDifference(
        TCR_After,
        th
          .normalizeColl18Dedc(totalCollAfter, coll0Decimals)
          .mul(price)
          .div(totalDebtAfter),
      ),
      1000,
  ...
  it("liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
   ...
   check that Bob's raw coll and debt has not changed, and that his "raw" ICR is above the MCR */
    ...
    const bob_rawICR = th
      .normalizeColl18Dedc(bob_Coll, coll0Decimals)
      .mul(th.toBN(dec(100, 18)))
      .div(bob_Debt);
  ...
  it("liquidateTroves(): emits liquidation event with correct values when all troves have ICR > MCR% and Stability Pool covers a subset of troves", async () => {
    ...
    assert.isTrue(
      (
        await troveManager.getCurrentICR(greta, collaterals[0].address, price)
      ).gte(coll0MCR),
    );
    assert.isTrue(
      (
    ...
    // Attempt liqudation sequence
    const liquidationTx = await troveManager.liquidateTroves(
      collaterals[0].address,
      10,
    );
    ...
    th.assertIsApproximatelyEqual(
      ...
      th.applyLiquidationFee(
        th.unwrapCollNativeDec(
          F_totalDebt.add(G_totalDebt).mul(toBN(coll0MCR)).div(price),
          coll0Decimals,
        ),
      ),
    );
    ...
    const freddy_remainingCollateral = F_coll.sub(
      th.unwrapCollNativeDec(
        F_totalDebt.mul(th.toBN(coll0MCR)).div(price),
        coll0Decimals,
      ),
    );
    const greta_remainingCollateral = G_coll.sub(
      th.unwrapCollNativeDec(
        G_totalDebt.mul(th.toBN(coll0MCR)).div(price),
        coll0Decimals,
      ),
    );
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getUserCollateral(freddy, collaterals[0].address),
      freddy_remainingCollateral,
    );
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getUserCollateral(greta, collaterals[0].address),
      greta_remainingCollateral,
    );

    // can claim collateral
    const freddy_balanceBefore = th.toBN(
      await collaterals[0].balanceOf(freddy),
    );
  ...
  it("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
    ...
    const ICR_E = await troveManager.getCurrentICR(
      erin,
      collaterals[0].address,
      price,
    );
  ...
  it("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    const equivalentColl = th.unwrapCollNativeDec(
      A_totalDebt.add(B_totalDebt)
    ...
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getUserCollateral(alice, collaterals[0].address),
  ...
  it("liquidateTroves() with a non fullfilled liquidation: ICR of non liquidated trove does not change", async () => {
    ...
    const ICR_C_Before = await troveManager.getCurrentICR(
      carol,
      collaterals[0].address,
      price,
    );
    ...
    const ICR_C_After = await troveManager.getCurrentICR(
      carol,
      collaterals[0].address,
      price,
    );
  ...
  it("batchLiquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    th.assertIsApproximatelyEqual(liquidatedDebt, A_totalDebt.add(B_totalDebt));
    const equivalentColl = th.unwrapCollNativeDec(
      A_totalDebt.add(B_totalDebt).mul(coll0MCR).div(price),
      coll0Decimals,
    );
    th.assertIsApproximatelyEqual(
      liquidatedColl,
    ...
    const alice_remainingCollateral = A_coll.sub(
      th.unwrapCollNativeDec(
        A_totalDebt.mul(coll0MCR).div(price),
        coll0Decimals,
      ),
    );
    const bob_remainingCollateral = B_coll.sub(
  ...
  it("batchLiquidateTroves(), with MCR < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const bobColl_Before = (
      await troveManager.Troves(bob, collaterals[0].address)
    )[1];
    const carolColl_Before = (
      await troveManager.Troves(carol, collaterals[0].address)
    )[1];
    const dennisColl_Before = (
    ...
    const trovesToLiquidate = [dennis, bob, carol];
    await assertRevert(
      troveManager.batchLiquidateTroves(
        collaterals[0].address,
        trovesToLiquidate,
      ),
      "TroveManager: nothing to liquidate",
    ...
    const bobDebt_After = (
      ...
      await troveManager.getPendingLUSDDebtReward(bob, collaterals[0].address),
    );
    const carolDebt_After = (
      await troveManager.Troves(carol, collaterals[0].address)
    )[0].add(
      await troveManager.getPendingLUSDDebtReward(
        carol,
  ...
  it("batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size", async () => {
    ...
    const G_collBefore = (
      await troveManager.Troves(G, collaterals[0].address)
    )[1];
    const G_debtBefore = (
      await troveManager.Troves(G, collaterals[0].address)
    )[0];
    const H_collBefore = (
      await troveManager.Troves(H, collaterals[0].address)
    )[1];
    const H_debtBefore = (
      await troveManager.Troves(H, collaterals[0].address)
    )[0];
    const I_collBefore = (
      await troveManager.Troves(I, collaterals[0].address)
    )[1];
    const I_debtBefore = (
      await troveManager.Troves(I, collaterals[0].address)
    )[0];
    ...
    const ICR_F = await troveManager.getCurrentICR(
      F,
      collaterals[0].address,
      price,
    );
    const ICR_G = await troveManager.getCurrentICR(
      G,
      collaterals[0].address,
      price,
    );
    const ICR_H = await troveManager.getCurrentICR(
      H,
      collaterals[0].address,
      price,
    );
    const ICR_I = await troveManager.getCurrentICR(
      I,
      collaterals[0].address,
      price,
    );
    ...
    await assertRevert(
      troveManager.batchLiquidateTroves(collaterals[0].address, [G, H, I]),
      "TroveManager: nothing to liquidate",
    );
    ...
    assert.equal(
      G_collBefore.eq(await troveManager.Troves(G, collaterals[0].address))[1],
    );
    assert.equal(
      G_debtBefore.eq(await troveManager.Troves(G, collaterals[0].address))[0],
    );
    assert.equal(
      H_collBefore.eq(await troveManager.Troves(H, collaterals[0].address))[1],
    );
    assert.equal(
      H_debtBefore.eq(await troveManager.Troves(H, collaterals[0].address))[0],
    );
    assert.equal(
      I_collBefore.eq(await troveManager.Troves(I, collaterals[0].address))[1],
    );
    assert.equal(
      I_debtBefore.eq(await troveManager.Troves(I, collaterals[0].address))[0],
    );
    ...
    await priceFeed.setPrice(collaterals[0].address, dec(110, 18));
    await stabilityPool.provideToSP(B_totalDebt.add(toBN(dec(50, 18))), {
      from: whale,
    });
    ...
    const dEbtBefore = (
      await troveManager.Troves(E, collaterals[0].address)
    )[0];
  ...
  it("batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > MCR and Stability Pool covers a subset of troves", async () => {
    ...
    th.assertIsApproximatelyEqual(
      ...
      th.applyLiquidationFee(
        th.unwrapCollNativeDec(
          F_totalDebt.add(G_totalDebt).mul(coll0MCR).div(price),
          coll0Decimals,
        ),
      ),
    );
    ...
    const freddy_remainingCollateral = F_coll.sub(
      th.unwrapCollNativeDec(
        F_totalDebt.mul(coll0MCR).div(price),
        coll0Decimals,
      ),
    );
    const greta_remainingCollateral = G_coll.sub(
      th.unwrapCollNativeDec(
        G_totalDebt.mul(coll0MCR).div(price),
        coll0Decimals,
      ),
    );
    th.assertIsApproximatelyEqual(
