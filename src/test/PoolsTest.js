...
contract("ActivePool", async (accounts) => {
  ...
  it("vault addresses are set correctly", async () => {
    assert.equal(
      await activePool.yieldGenerator(collaterals[0].address),
      vaults[0].address,
    );
    assert.equal(
      await activePool.yieldGenerator(collaterals[1].address),
      vaults[1].address,
    );
  });
  ...
  it("default yielding percentages are 0", async () => {
    assert.equal(
      (await activePool.yieldingPercentage(collaterals[0].address)).toString(),
      "0",
    );
    assert.equal(
      (await activePool.yieldingPercentage(collaterals[1].address)).toString(),
      "0",
    );
  });
  ...
  it("default yielding amounts are 0", async () => {
    assert.equal(
      (await activePool.yieldingAmount(collaterals[0].address)).toString(),
      "0",
    );
    assert.equal(
      (await activePool.yieldingAmount(collaterals[1].address)).toString(),
      "0",
    );
  });
  ...
  it("default yield claim thresholds are 0", async () => {
    assert.equal(
      (await activePool.yieldClaimThreshold(collaterals[0].address)).toString(),
      "0",
    );
    assert.equal(
      (await activePool.yieldClaimThreshold(collaterals[1].address)).toString(),
      "0",
    );
  });
  ...
  it("default yield percentage drift is 1%", async () => {
    assert.equal(
      (await activePool.yieldingPercentageDrift()).toString(),
      "100",
    );
  });
  ...
  it("manualRebalance works with default values, vault share bal is 0 before and after", async () => {
    ...
    await activePool.manualRebalance(collaterals[0].address, dec(1, "ether"), {
      from: owner,
    });
    ...
    await activePool.manualRebalance(collaterals[0].address, dec(2, "ether"), {
      from: owner,
    });

    const activePool_CollBalFinal = await collaterals[0].balanceOf(
      activePool.address,
    );
    const activePool_VaultBalFinal = await vaults[0].balanceOf(
      activePool.address,
    );
  ...
  it("non-owner cannot update yielding percentage for any collateral", async () => {
    try {
      const txAlice = await activePool.setYieldingPercentage(
        collaterals[0].address,
        4000,
        { from: alice },
      );
  ...
  it("owner cannot set yielding percentage for any collateral > 10k", async () => {
    try {
      const txOwner = await activePool.setYieldingPercentage(
        collaterals[0].address,
        10001,
        { from: owner },
      );
  ...
  it("non-owner cannot update yielding percentage drift", async () => {
    try {
      const txAlice = await activePool.setYieldingPercentageDrift(400, {
        from: alice,
      });
    } catch (err) {
      assert.include(err.message, "revert");
  ...
  it("owner cannot set yielding percentage drift > 500", async () => {
    try {
      const txOwner = await activePool.setYieldingPercentageDrift(501, {
        from: owner,
      });
    } catch (err) {
      assert.include(err.message, "revert");
  ...
  it("only owner can set yielding percentage drift <= 500", async () => {
    const txOwner = await activePool.setYieldingPercentageDrift(400, {
      from: owner,
    });
    assert.equal(
      (await activePool.yieldingPercentageDrift()).toString(),
  ...
  it("non-owner cannot update yield claim threshold for any collateral", async () => {
    try {
      const txAlice = await activePool.setYieldClaimThreshold(
        collaterals[0].address,
        dec(1, "ether"),
        { from: alice },
      );
  ...
  it("only owner can set yield claim threshold for any collateral", async () => {
    const txOwner = await activePool.setYieldClaimThreshold(
      collaterals[0].address,
      dec(1, "ether"),
      { from: owner },
    );
    assert.equal(
      await activePool.yieldClaimThreshold(collaterals[0].address),
      dec(1, "ether"),
    );
  });
  ...
  it("non-owner cannot call manualRebalance", async () => {
    try {
      const txAlice = await activePool.manualRebalance(
        collaterals[0].address,
        dec(1, "ether"),
        { from: alice },
      );
  ...
  it("only owner can call manualRebalance", async () => {
    const txOwner = await activePool.manualRebalance(
      collaterals[0].address,
      dec(0, "ether"),
      { from: owner },
    );
  ...
  it("reasonableDefaultState: verify yielding amounts + vault share balances are correct", async () => {
    ...
    assert.equal(
      await activePool.yieldingAmount(collaterals[0].address),
      dec(5, "ether"),
    );
  ...
  it("simulate profit, check that send collateral distributes profit", async () => {
    ...
    assert.equal(
      (await collaterals[0].balanceOf(treasury.address)).toString(),
      "1000000000000000000",
    ); // 1 ether
    ...
    assert.equal(
      await collaterals[0].balanceOf(vaults[0].address),
      "4500000000000000000",
    );
  });
  ...
  it("simulate profit and increase yielding percentage, check that send collateral distributes profit", async () => {
    ...
    await activePool.setYieldingPercentage(collaterals[0].address, 6000, {
      from: owner,
    });
  ...
  it("simulate profit and reduce yielding percentage, check that send collateral distributes profit", async () => {
    ...
    await activePool.setYieldingPercentage(collaterals[0].address, 3500, {
      from: owner,
    });
    ...
    const sendCollData2 = th.getTransactionData(
      "sendCollateral(address,address,uint256)",
      [collaterals[0].address, alice, web3.utils.toHex(dec(1, "ether"))],
    );
    await mockBorrowerOperations.forward(activePool.address, sendCollData2, {
      from: owner,
    });
  ...
  it("simulate profit and reduce yielding percentage, check that pull collateral distributes profit", async () => {
    ...
    const pullCollData2 = th.getTransactionData(
      "pullCollateralFromBorrowerOperationsOrDefaultPool(address,uint256)",
      [collaterals[0].address, web3.utils.toHex(dec(7, "ether"))],
    );
    await mockBorrowerOperations.forward(activePool.address, pullCollData2);
  ...
  it("simulate profit, check that manual rebalance distributes profit", async () => {
    ...
    await activePool.manualRebalance(collaterals[0].address, dec(0, "ether"), {
      from: owner,
    });
  ...
  it("simulate profit, update treasury", async () => {
    ...
    assert.equal(
      (await collaterals[0].balanceOf(newTreasury.address)).toString(),
      "2000000000000000000",
    );
  ...
  it("simulate profit and increase yielding percentage, check that manual rebalance distributes profit", async () => {
    ...
    await activePool.setYieldingPercentage(collaterals[0].address, 7500, {
      from: owner,
    });
  ...
  it("simulate profit and reduce yielding percentage, check that manual rebalance distributes profit", async () => {
    ...
    // reduce yielding percentage
    await activePool.setYieldingPercentage(collaterals[0].address, 4000, {
      from: owner,
    });
    ...
    await activePool.setYieldingPercentage(collaterals[0].address, 2500, {
      from: owner,
    });
  ...
  it("simulate loss, check that manual rebalance reverts", async () => {
    ...
    try {
      // trigger manual rebalance
      await activePool.manualRebalance(
        collaterals[0].address,
        dec(0, "ether"),
        { from: owner },
      );
