...
contract("CollateralConfig", async (accounts) => {
  ...
  it("sets the right values on initializing", async () => {
    ...
    expect(await collateralConfig.isCollateralAllowed(collaterals[0].address))
      .to.be.true;
    expect(await collateralConfig.isCollateralAllowed(collaterals[1].address))
      .to.be.true;
    expect(await collateralConfig.isCollateralAllowed(accounts[0])).to.be.false;

    expect(
      await collateralConfig.getCollateralDecimals(collaterals[0].address),
    ).to.eql(toBN("12"));
    expect(
      await collateralConfig.getCollateralDecimals(collaterals[1].address),
    ).to.eql(toBN("8"));
    await th.assertRevert(
      collateralConfig.getCollateralDecimals(accounts[0]),
      "Invalid collateral address",
    );

    expect(
      await collateralConfig.getCollateralMCR(collaterals[0].address),
    ).to.eql(toBN(dec(120, 16)));
    expect(
      await collateralConfig.getCollateralMCR(collaterals[1].address),
    ).to.eql(toBN(dec(130, 16)));
    await th.assertRevert(
      collateralConfig.getCollateralMCR(accounts[0]),
      "Invalid collateral address",
    );

    expect(
      await collateralConfig.getCollateralCCR(collaterals[0].address),
    ).to.eql(toBN(dec(165, 16)));
    expect(
      await collateralConfig.getCollateralCCR(collaterals[1].address),
    ).to.eql(toBN(dec(180, 16)));
    await th.assertRevert(
      collateralConfig.getCollateralCCR(accounts[0]),
      "Invalid collateral address",
    );
  });
  ...
  it("owner can update CRs but only by lowering them", async () => {
    ...
    expect(
      await collateralConfig.getCollateralMCR(collaterals[0].address),
    ).to.eql(toBN(dec(115, 16)));
    expect(
      await collateralConfig.getCollateralCCR(collaterals[0].address),
    ).to.eql(toBN(dec(155, 16)));
  ...
  it("debt limit is enforced", async () => {
    ...
    const troveDebt = await troveManager.getTroveDebt(
      alice,
      collaterals[0].address,
    );
    th.assertIsApproximatelyEqual(
      troveDebt,
      toBN(dec(199, 18)).add(LUSD_GAS_COMPENSATION),
      Number(dec(1, 18)),
    );
  });
