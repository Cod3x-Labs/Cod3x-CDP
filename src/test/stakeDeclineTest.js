  ...
  it("A given trove's stake decline is negligible with adjustments and tiny liquidations", async () => {
    ...
    assert.isTrue(
      await troveManager.checkRecoveryMode(
        collaterals[0].address,
        await priceFeed.getPrice(collaterals[0].address),
      ),
    );
    await troveManager.liquidate(A, collaterals[0].address);

    console.log(
      `totalStakesSnapshot after L1: ${await troveManager.totalStakesSnapshot(collaterals[0].address)}`,
    );
    console.log(
      `totalCollateralSnapshot after L1: ${await troveManager.totalCollateralSnapshot(collaterals[0].address)}`,
    );
    console.log(`Snapshots ratio after L1: ${await getSnapshotsRatio()}`);
    console.log(
      `B pending ETH reward after L1: ${await troveManager.getPendingCollateralReward(B, collaterals[0].address)}`,
    );
    console.log(
      `B stake after L1: ${(await troveManager.Troves(B, collaterals[0].address))[2]}`,
    );
    ...
    console.log(
      `B stake after A1: ${(await troveManager.Troves(B, collaterals[0].address))[2]}`,
    );
    ...
    for (let [idx, trove] of tinyTroves.entries()) {
      ...
      console.log(
        `B stake after L${idx + 2}: ${(await troveManager.Troves(B, collaterals[0].address))[2]}`,
      );
      console.log(
      ...
      console.log(
        `B stake after A${idx + 2}: ${(await troveManager.Troves(B, collaterals[0].address))[2]}`,
      );
    }
  });
