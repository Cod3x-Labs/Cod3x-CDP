  ...
  async (accounts) => {
    ...
    context("Batch liquidations", () => {
      ...
      it("Stability Pool profit matches", async () => {
        ...
        const spEthBefore = await stabilityPool.getCollateral(
          collaterals[0].address,
        );
        ...
        if (collDecimals.lt(bn_18)) {
          expectedCollateralLiquidatedA = expectedCollateralLiquidatedA.div(
            toBN(10).pow(bn_18.sub(collDecimals)),
          );
        } else if (collDecimals.gt(bn_18)) {
          expectedCollateralLiquidatedA = expectedCollateralLiquidatedA.mul(
            toBN(10).pow(collDecimals.sub(bn_18)),
          );
        }
    ...
    context("Sequential liquidations", () => {
      ...
      it("First trove only doesn’t get out of Recovery Mode", async () => {
        await setup();
        const tx = await troveManager.liquidateTroves(
          collaterals[0].address,
          1,
        );
