...
contract("StabilityPool", async (accounts) => {
  ...
  describe("Stability Pool Mechanisms", async () => {
    ...
    beforeEach(async () => {
      ...
      const LQTYContracts =
        await deploymentHelper.deployLQTYContracts(multisig);
    ...
    it("provideToSP(): Correctly updates user snapshots of accumulated rewards per unit staked", async () => {
      ...
      assert.isFalse(
        await sortedTroves.contains(collaterals[1].address, defaulter_1),
      );
      assert.isFalse(
        await sortedTroves.contains(collaterals[1].address, defaulter_2),
      );
      ...
      const alice_snapshot_S_Before = (
        await stabilityPool.depositSnapshots_S(alice, collaterals[1].address)
      ).toString();
      ...
      const alice_snapshot_S_After = await stabilityPool.depositSnapshots_S(
        alice,
        collaterals[1].address,
      );
    ...
    it("provideToSP(), multiple deposits: updates user's deposit and snapshots", async () => {
      ...
      await openTrove({
        collateral: collaterals[0],
        value: toBN(dec(500, collDecimals)),
        extraLUSDAmount: toBN(dec(10000, 18)),
        extraParams: { from: whale },
      });
      const whaleLUSD = await lusdToken.balanceOf(whale);
      ...
      await openTrove({
        collateral: collaterals[0],
        extraLUSDAmount: toBN(dec(850, 18)),
        ICR: toBN(dec(3, 18)),
        extraParams: { from: alice },
      });
      ...
      const alice_Snapshot_S_0 = await stabilityPool.depositSnapshots_S(
        alice,
        collaterals[0].address,
      );
      ...
      const alice_Snapshot_S_1 = await stabilityPool.depositSnapshots_S(
        alice,
        collaterals[0].address,
      );
      ...
      const alice_Snapshot_S_2 = await stabilityPool.depositSnapshots_S(
        alice,
        collaterals[0].address,
      );
    ...
    it("provideToSP(): doesn't impact other users' deposits or collateral gains", async () => {
      ...
      const alice_BTCGain_Before = (
        await stabilityPool.getDepositorCollateralGain(alice)
      )[1][1].toString(); // [[assets], [amounts]]
      const bob_BTCGain_Before = (
        await stabilityPool.getDepositorCollateralGain(bob)
      )[1][1].toString(); // [[assets], [amounts]]
      const carol_BTCGain_Before = (
      ...
      const alice_BTCGain_After = (
        await stabilityPool.getDepositorCollateralGain(alice)
      )[1][1].toString(); // [[assets], [amounts]]
      const bob_BTCGain_After = (
        await stabilityPool.getDepositorCollateralGain(bob)
      )[1][1].toString(); // [[assets], [amounts]]
      const carol_BTCGain_After = (
        await stabilityPool.getDepositorCollateralGain(carol)
      )[1][1].toString(); // [[assets], [amounts]]
    ...
    it("withdrawFromSP(): Subsequent deposit and withdrawal attempt from same account, with no intermediate liquidations, withdraws zero ETH", async () => {
      ...
      await stabilityPool.withdrawFromSP(dec(15000, 18), { from: alice });
      assert.equal(
        (await stabilityPool.getDepositorCollateralGain(alice))[1].length,
        0,
      );
      ...
      assert.equal(
        (await stabilityPool.getDepositorCollateralGain(alice))[1][0],
        0,
      );
    ...
    it("withdrawFromSP(): it correctly updates the user's LUSD and ETH snapshots of entitled reward per unit staked", async () => {
      ...
      const alice_snapshot_Before = await stabilityPool.depositSnapshots(alice);
      const alice_snapshot_S_Before = (
        await stabilityPool.depositSnapshots_S(alice, collaterals[0].address)
      ).toString();
      const alice_snapshot_P_Before = alice_snapshot_Before[0].toString();
      assert.equal(alice_snapshot_S_Before, 0);
      ...
      const S = (
        await stabilityPool.epochToScaleToSum(0, 0, collaterals[0].address)
      ).toString();
      // check 'After' snapshots
      const alice_snapshot_After = await stabilityPool.depositSnapshots(alice);
    ...
    it("withdrawFromSP(): increases depositor's LUSD token balance by the expected amount", async () => {
      ...
      const defaulterDebt = (
        await troveManager.getEntireDebtAndColl(
          defaulter_1,
          collaterals[0].address,
        )
      )[0];
    ...
    it("withdrawFromSP(): succeeds when amount is 0 and system has an undercollateralized trove", async () => {
      ...
      const A_pendingETHGain = (
        await stabilityPool.getDepositorCollateralGain(A)
      )[1][0];
    ...
    it("withdrawFromSP(): caller can withdraw full deposit and ETH gain during Recovery Mode", async () => {
      ...
      const alice_ETH_Balance_Before = web3.utils.toBN(
        await collaterals[0].balanceOf(alice),
      );
      const bob_ETH_Balance_Before = web3.utils.toBN(
        await collaterals[0].balanceOf(bob),
      );
      const carol_ETH_Balance_Before = web3.utils.toBN(
        await collaterals[0].balanceOf(carol),
      );
