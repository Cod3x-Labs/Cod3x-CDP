...
contract(
  ...
  async (accounts) => {
    ...
    before(async () => {
      ...
      coreContracts =
        await deploymentHelper.deployLUSDTokenTester(coreContracts);
      coreContracts =
        await deploymentHelper.deployTestCollaterals(coreContracts);
      const LQTYContracts =
    ...
    describe("TroveManager", async (accounts) => {
      ...
      it("applyPendingRewards(): reverts when called by an account that is not BorrowerOperations", async () => {
        ...
        try {
          const txAlice = await troveManager.applyPendingRewards(
            bob,
      ...
      it("updateRewardSnapshots(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.updateTroveRewardSnapshots(
            bob,
            collaterals[0].address,
            { from: alice },
          );
      ...
      it("removeStake(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.removeStake(
            bob,
            collaterals[0].address,
            { from: alice },
          );
      ...
      it("updateStakeAndTotalStakes(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.updateStakeAndTotalStakes(
            bob,
            collaterals[0].address,
            { from: alice },
          );
      ...
      it("closeTrove(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.closeTrove(
            bob,
            collaterals[0].address,
            2,
            { from: alice },
      ...
      it("addTroveOwnerToArray(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.addTroveOwnerToArray(
            bob,
            collaterals[0].address,
            { from: alice },
          );
      ...
      it("setTroveStatus(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.setTroveStatus(
            bob,
            collaterals[0].address,
            1,
            { from: alice },
      ...
      it("increaseTroveColl(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.increaseTroveColl(
            bob,
            collaterals[0].address,
            100,
            { from: alice },
      ...
      it("decreaseTroveColl(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.decreaseTroveColl(
            bob,
            collaterals[0].address,
            100,
            { from: alice },
      ...
      it("increaseTroveDebt(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.increaseTroveDebt(
            bob,
            collaterals[0].address,
            100,
            { from: alice },
      ...
      it("decreaseTroveDebt(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.decreaseTroveDebt(
            bob,
            collaterals[0].address,
            100,
            { from: alice },
    ...
    describe("RedemptionHelper", async (accounts) => {
      it("redeemCollateral(): reverts when called by an account that it not TroveManager", async () => {
        ...
        const redemptionhint =
          await coreContracts.hintHelpers.getRedemptionHints(
            collaterals[0].address,
            toBN(dec(50, 18)),
            price,
            0, // gas price
        ...
        const exactPartialRedemptionHint =
          await coreContracts.sortedTroves.findInsertPosition(
            collaterals[0].address,
            partialRedemptionNewICR,
    ...
    describe("ActivePool", async (accounts) => {
      ...
      it("sendCollateral(): reverts when called by an account that is not BO nor TroveM nor SP nor LH", async () => {
        // Attempt call from alice
        try {
          const txAlice = await activePool.sendCollateral(
            collaterals[0].address,
            alice,
            100,
            { from: alice },
          ...
          assert.include(
            err.message,
            "Caller is neither BO nor TroveM nor SP nor LH",
          );
        }
      });
      ...
      it("increaseLUSDDebt(): reverts when called by an account that is not BO nor TroveM", async () => {
        // Attempt call from alice
        try {
          const txAlice = await activePool.increaseLUSDDebt(
            collaterals[0].address,
            100,
            { from: alice },
          );
      ...
      it("decreaseLUSDDebt(): reverts when called by an account that is not BO nor TroveM nor SP nor RH", async () => {
        // Attempt call from alice
        try {
          const txAlice = await activePool.decreaseLUSDDebt(
            collaterals[0].address,
            100,
            { from: alice },
          );
          ...
          assert.include(
            err.message,
            "Caller is neither BO nor TroveM nor SP nor RH",
          );
        }
      });
      ...
      it("pullCollateral(): reverts when called by an account that is not Borrower Operations nor Default Pool", async () => {
        ...
        try {
          const txAlice =
            await activePool.pullCollateralFromBorrowerOperationsOrDefaultPool(
              collaterals[0].address,
              100,
              { from: alice },
            );
    ...
    describe("DefaultPool", async (accounts) => {
      ...
      it("sendCollateralToActivePool(): reverts when called by an account that is not TroveManager", async () => {
        // Attempt call from alice
        try {
          const txAlice = await defaultPool.sendCollateralToActivePool(
            collaterals[0].address,
            100,
            { from: alice },
          );
      ...
      it("increaseLUSDDebt(): reverts when called by an account that is not TroveManager", async () => {
        // Attempt call from alice
        try {
          const txAlice = await defaultPool.increaseLUSDDebt(
            collaterals[0].address,
            100,
            { from: alice },
          );
      ...
      it("decreaseLUSD(): reverts when called by an account that is not TroveManager", async () => {
        // Attempt call from alice
        try {
          const txAlice = await defaultPool.decreaseLUSDDebt(
            collaterals[0].address,
            100,
            { from: alice },
          );
      ...
      it("pullCollateral(): reverts when called by an account that is not the Active Pool", async () => {
        // Attempt call from alice
        try {
          const txAlice = await defaultPool.pullCollateralFromActivePool(
            collaterals[0].address,
            100,
            { from: alice },
          );
    ...
    describe("SortedTroves", async (accounts) => {
      ...
      it("remove(): reverts when called by an account that is not TroveManager", async () => {
        // Attempt call from alice
        try {
          const txAlice = await sortedTroves.remove(
            collaterals[0].address,
            bob,
            { from: alice },
          );
    ...
    describe("CommunityIssuance", async (accounts) => {
      it("sendOath(): reverts when caller is not the StabilityPool", async () => {
        ...
        const tx2 = communityIssuance.sendOath(bob, dec(100, 18), {
          from: alice,
        });
        const tx3 = communityIssuance.sendOath(
