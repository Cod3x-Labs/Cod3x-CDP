...
contract("TroveManager", async (accounts) => {
  ...
  const mintCollateralAndApproveBorrowerOps = async (
    collateral,
    user,
    amount,
  ) => {
  ...
  beforeEach(async () => {
    ...
    await oathToken.approve(communityIssuance.address, toBN(dec(14000, 18)), {
      from: owner,
    });
  ...
  it("liquidate(): closes a Trove that has ICR < MCR", async () => {
    ...
    const MCR = (
      await contracts.collateralConfig.getCollateralMCR(collaterals[0].address)
    ).toString();
    ...
    const status = (
      await troveManager.Troves(alice, collaterals[0].address)
    )[3];
    assert.equal(status, 3); // status enum 3 corresponds to "Closed by liquidation"
    const alice_Trove_isInSortedList = await sortedTroves.contains(
      collaterals[0].address,
      alice,
    );
  ...
  it("liquidate(): decreases ActivePool ETH and LUSDDebt by correct amounts", async () => {
    ...
    const activePool_ETH_Before = (
      await activePool.getCollateral(collaterals[0].address)
    ).toString();
    const activePool_RawEther_Before = (
      await collaterals[0].balanceOf(activePool.address)
    ).toString();
    const activePool_LUSDDebt_Before = (
      await activePool.getLUSDDebt(collaterals[0].address)
    ).toString();
  ...
  it("liquidate(): increases DefaultPool ETH and LUSD debt by correct amounts", async () => {
    ...
    const defaultPool_RawEther_Before = (
      await collaterals[0].balanceOf(defaultPool.address)
    ).toString();
    const defaultPool_LUSDDebt_Before = (
      await defaultPool.getLUSDDebt(collaterals[0].address)
    ).toString();
    ...
    const defaultPool_ETH_After = (
      await defaultPool.getCollateral(collaterals[0].address)
    ).toString();
  ...
  it("liquidate(): removes the Trove's stake from the total stakes", async () => {
    ...
    const totalStakes_Before = (
      await troveManager.totalStakes(collaterals[0].address)
    ).toString();
  ...
  it("liquidate(): Removes the correct trove from the TroveOwners array, and moves the last array element to the new empty slot", async () => {
    ...
    const whale_arrayIndex = (
      await troveManager.Troves(whale, collaterals[0].address)
    )[4];
    const alice_arrayIndex = (
      await troveManager.Troves(alice, collaterals[0].address)
    )[4];
    const bob_arrayIndex = (
      await troveManager.Troves(bob, collaterals[0].address)
    )[4];
    const dennis_arrayIndex = (
      await troveManager.Troves(dennis, collaterals[0].address)
    )[4];
    const erin_arrayIndex = (
      await troveManager.Troves(erin, collaterals[0].address)
    )[4];
  ...
  it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
    ...
    const totalStakesSnapshot_Before = (
      await troveManager.totalStakesSnapshot(collaterals[0].address)
    ).toString();
    const totalCollateralSnapshot_Before = (
      await troveManager.totalCollateralSnapshot(collaterals[0].address)
    ).toString();
  ...
  it("liquidate(): updates the L_ETH and L_LUSDDebt reward-per-unit-staked totals", async () => {
    ...
    const L_ETH_AfterCarolLiquidated = await troveManager.L_Collateral(
      collaterals[0].address,
    );
    ...
    const L_ETH_expected_1 = th
      ...
      .mul(toBN(10).pow(collDecimals))
      .div(A_collateral.add(B_collateral));
    const L_LUSDDebt_expected_1 = C_totalDebt.mul(
      toBN(10).pow(collDecimals),
    ).div(A_collateral.add(B_collateral));
    assert.isAtMost(
   ...
   L_LUSDDebt = (180 / 20) + (890 / 10) = 98 LUSD */
    const L_ETH_AfterBobLiquidated = await troveManager.L_Collateral(
      collaterals[0].address,
    );
    ...
    const L_ETH_expected_2 = L_ETH_expected_1.add(
      th
        .applyLiquidationFee(
          B_collateral.add(
            B_collateral.mul(L_ETH_expected_1).div(toBN(10).pow(collDecimals)),
          ),
        )
        .mul(toBN(10).pow(collDecimals))
        .div(A_collateral),
    ...
    const L_LUSDDebt_expected_2 = L_LUSDDebt_expected_1.add(
      B_totalDebt.add(B_increasedTotalDebt)
        .add(
          th
            .normalizeColl18Dedc(B_collateral, collDecimals)
            .mul(L_LUSDDebt_expected_1)
            .div(mv._1e18BN),
        )
        .mul(mv._1e18BN)
        .div(th.normalizeColl18Dedc(A_collateral, collDecimals)),
    );
    assert.isAtMost(
  ...
  it("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
    ...
    const alice_ICR = (
      await troveManager.getCurrentICR(alice, collaterals[0].address, price)
    ).toString();
    ...
    const alice_isInSortedList = await sortedTroves.contains(
      collaterals[0].address,
      alice,
    );
    assert.isFalse(alice_isInSortedList);

    const bob_isInSortedList = await sortedTroves.contains(
      collaterals[0].address,
      bob,
    );
  ...
  it("liquidate(): reverts if trove is non-existent", async () => {
    ...
    assert.equal(
      await troveManager.getTroveStatus(carol, collaterals[0].address),
      0,
  ...
  it("liquidate(): reverts if trove has been closed", async () => {
    ...
    const txCarol_L1 = await troveManager.liquidate(
      carol,
      collaterals[0].address,
    );
    ...
    assert.equal(
      await troveManager.getTroveStatus(carol, collaterals[0].address),
      3,
    ); // check trove closed by liquidation
    ...
    try {
      const txCarol_L2 = await troveManager.liquidate(
        carol,
        collaterals[0].address,
      );
  ...
  it("liquidate(): does nothing if trove has >= MCR ICR", async () => {
    ...
    const TCR_Before = (
      await th.getTCR(contracts, collaterals[0].address)
    ).toString();
    const listSize_Before = (
      await sortedTroves.getSize(collaterals[0].address)
    ).toString();

    const price = await priceFeed.getPrice(collaterals[0].address);

    // Check Bob's ICR > MCR
    const collMCR = await contracts.collateralConfig.getCollateralMCR(
      collaterals[0].address,
    );
    ...
    await assertRevert(
      troveManager.liquidate(bob, collaterals[0].address),
  ...
  it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
    ...
    assert.isTrue(
      await sortedTroves.contains(collaterals[0].address, defaulter_1),
    );
    assert.isTrue(
      await sortedTroves.contains(collaterals[0].address, defaulter_2),
    );
    assert.isTrue(
      await sortedTroves.contains(collaterals[0].address, defaulter_3),
    );
    assert.isTrue(
      await sortedTroves.contains(collaterals[0].address, defaulter_4),
    );
  ...
  it("liquidate(): a pure redistribution reduces the TCR only as a result of compensation", async () => {
    ...
    const expectedTCR_0 = th
      .normalizeColl18Dedc(entireSystemCollBefore, collDecimals)
      .mul(price)
    ...
    const expectedTCR_1 = th
      .normalizeColl18Dedc(entireSystemCollBefore.sub(gasComp_1), collDecimals)
      .mul(price)
      .div(entireSystemDebtBefore);
    ...
    const expectedTCR_2 = th
      .normalizeColl18Dedc(
        entireSystemCollBefore.sub(gasComp_1).sub(gasComp_2),
        collDecimals,
      )
      .mul(price)
      .div(entireSystemDebtBefore);
    ...
    const liquidationTx_3 = await troveManager.liquidate(
      defaulter_3,
      collaterals[0].address,
    );
    ...
    const liquidationTx_4 = await troveManager.liquidate(
      defaulter_4,
      collaterals[0].address,
    );
    ...
    const expectedTCR_4 = th
      .normalizeColl18Dedc(
        entireSystemCollBefore
          .sub(gasComp_1)
          .sub(gasComp_2)
          .sub(gasComp_3)
          .sub(gasComp_4),
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await priceFeed.setPrice(collaterals[0].address, dec(100, 18));
    const liquidationTX_C = await troveManager.liquidate(
      carol,
      collaterals[0].address,
    );
    ...
    const dennis_ETHGain_Before = (
      await stabilityPool.getDepositorCollateralGain(dennis)
    )[1][0].toString();
    assert.isAtMost(
  ...
  it("liquidate(): does not liquidate a SP depositor's trove with ICR > MCR, and does not affect their SP deposit or ETH gain", async () => {
    ...
    assert.isTrue(
      (await troveManager.getCurrentICR(bob, collaterals[0].address, price)).gt(
        collMCR,
      ),
    );
    ...
    const bob_ETHGain_Before = (
      await stabilityPool.getDepositorCollateralGain(bob)
    )[1][0].toString();
    assert.isAtMost(
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < MCR, and the liquidation correctly impacts their SP deposit and ETH gain", async () => {
    ...
    const alice_ETHGain_After = (
      await stabilityPool.getDepositorCollateralGain(alice)
    )[1][0].toString();
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    ...
    assert.equal(
      (await sortedTroves.getSize(collaterals[0].address)).toString(),
      "4",
    );
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    */
    ...
    const bob_rawICR = th
      .normalizeColl18Dedc(bob_Coll, collDecimals)
      .mul(toBN(dec(100, 18)))
      .div(bob_Debt);
    ...
    await assertRevert(
      troveManager.liquidate(alice, collaterals[0].address),
      "TroveManager: nothing to liquidate",
    );
    await troveManager.liquidate(bob, collaterals[0].address);
    await troveManager.liquidate(carol, collaterals[0].address);
   ...
   (because his pending rewards bring his ICR < MCR) */
    ...
    assert.equal(
      (await troveManager.Troves(alice, collaterals[0].address))[3].toString(),
      "1",
    );
    assert.equal(
      (await troveManager.Troves(bob, collaterals[0].address))[3].toString(),
      "3",
    );
    assert.equal(
      (await troveManager.Troves(carol, collaterals[0].address))[3].toString(),
      "3",
    );
  });
  ...
  it("liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards", async () => {
    ...
    const ICR_C = await troveManager.getCurrentICR(
      C,
      collaterals[0].address,
      price,
    );
    ...
    const liqTxBC = await troveManager.liquidateTroves(
      collaterals[0].address,
      2,
    );
    ...
    assert.isTrue(
      (await sortedTroves.getSize(collaterals[0].address)).eq(toBN("3")),
    );
    ...
    assert.isTrue(
      await troveManager.hasPendingRewards(C, collaterals[0].address),
    );
    assert.isFalse(
      await troveManager.hasPendingRewards(D, collaterals[0].address),
    );
    assert.isFalse(
      await troveManager.hasPendingRewards(E, collaterals[0].address),
    );

    // Check C's pending coll and debt rewards are <= the coll and debt in the DefaultPool
    const pendingETH_C = await troveManager.getPendingCollateralReward(
      C,
      collaterals[0].address,
    );
    const pendingLUSDDebt_C = await troveManager.getPendingLUSDDebtReward(
      C,
      collaterals[0].address,
    );
    ...
    const liqTx2 = await troveManager.liquidateTroves(
      collaterals[0].address,
      2,
    );
    ...
    assert.isTrue(await sortedTroves.contains(collaterals[0].address, E));
    assert.isTrue(
      (await sortedTroves.getSize(collaterals[0].address)).eq(toBN("1")),
    );
  });
  ...
  it("liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves", async () => {
    ...
    assert.isTrue(
      (
        await troveManager.getCurrentICR(bob, collaterals[0].address, price)
      ).lte(collMCR),
    );
    assert.isTrue(
      (
        await troveManager.getCurrentICR(carol, collaterals[0].address, price)
      ).lte(collMCR),
    );
    assert.isTrue(
      (
        await troveManager.getCurrentICR(erin, collaterals[0].address, price)
      ).lte(collMCR),
    );
    assert.isTrue(
      (
        await troveManager.getCurrentICR(flyn, collaterals[0].address, price)
      ).lte(collMCR),
    );
    ...
    assert.isTrue(
      (
        await troveManager.getCurrentICR(graham, collaterals[0].address, price)
      ).gte(collMCR),
    );
    assert.isTrue(
      (
        await troveManager.getCurrentICR(harriet, collaterals[0].address, price)
      ).gte(collMCR),
    );
    assert.isTrue(
      (
        await troveManager.getCurrentICR(ida, collaterals[0].address, price)
      ).gte(collMCR),
    );
    ...
    assert.isTrue(
      (
        await troveManager.getCurrentICR(whale, collaterals[0].address, price)
      ).gte(collMCR),
    );
    ...
    assert.equal(
      (await troveManager.Troves(erin, collaterals[0].address))[3].toString(),
      "3",
    );
    assert.equal(
      (await troveManager.Troves(flyn, collaterals[0].address))[3].toString(),
      "3",
    );
  ...
  it("liquidateTroves(): liquidates  up to the requested number of undercollateralized troves", async () => {
    ...
    const aliceTroveStatus = (
      await troveManager.getTroveStatus(alice, collaterals[0].address)
    ).toString();
    const bobTroveStatus = (
      await troveManager.getTroveStatus(bob, collaterals[0].address)
    ).toString();
    ...
    const carol_isInSortedList = await sortedTroves.contains(
      collaterals[0].address,
      carol,
    );
    ...
    const dennisTroveStatus = (
      await troveManager.getTroveStatus(dennis, collaterals[0].address)
    ).toString();
    const erinTroveStatus = (
      await troveManager.getTroveStatus(erin, collaterals[0].address)
    ).toString();
    ...
    const dennis_isInSortedList = await sortedTroves.contains(
      collaterals[0].address,
      dennis,
    );
    const erin_isInSortedList = await sortedTroves.contains(
      collaterals[0].address,
      erin,
    );
  ...
  it("liquidateTroves(): does nothing if all troves have ICR > MCR", async () => {
    ...
    await assertRevert(
      troveManager.liquidateTroves(collaterals[0].address, 10),
      "TroveManager: nothing to liquidate",
    );
  ...
  it("liquidateTroves(): reverts if n = 0", async () => {
    ...
    await assertRevert(
      troveManager.liquidateTroves(collaterals[0].address, 0),
      "TroveManager: nothing to liquidate",
    );
  ...
  it("liquidateTroves():  liquidates troves with ICR < MCR", async () => {
    ...
    const flyn_ICR = await troveManager.getCurrentICR(
      flyn,
      collaterals[0].address,
      price,
    );
  ...
  it("liquidateTroves(): A liquidation sequence of pure redistributions decreases the TCR, due to gas compensation, but up to 0.5%", async () => {
    ...
    assert.isAtMost(
      th.getDifference(
        TCR_Before,
        th
          .normalizeColl18Dedc(totalColl, collDecimals)
          .mul(price)
          .div(totalDebt),
      ),
      1000,
    ...
    assert.isAtMost(
      th.getDifference(
        ...
        th
          .normalizeColl18Dedc(
            totalCollNonDefaulters.add(
              th.applyLiquidationFee(totalCollDefaulters),
            ),
            collDecimals,
          )
          .mul(price)
  ...
  it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
    ...
    const whale_ETHGain = (
      await stabilityPool.getDepositorCollateralGain(whale)
    )[1][0];
    const alice_ETHGain = (
      await stabilityPool.getDepositorCollateralGain(alice)
    )[1][0];
    ...
    const total_ETHinSP = (
      await stabilityPool.getCollateral(collaterals[0].address)
    ).toString();
  ...
  it("batchLiquidateTroves(): closes every trove with ICR < MCR in the given array", async () => {
    ...
    assert.isTrue(
      (await troveManager.getCurrentICR(bob, collaterals[0].address, price)).lt(
        collMCR,
      ),
    );
    assert.isTrue(
    ...
    assert.isTrue(
      (
        await troveManager.getCurrentICR(dennis, collaterals[0].address, price)
      ).gte(collMCR),
    );
    assert.isTrue(
      (
  ...
  it("batchLiquidateTroves(): does not liquidate troves that are not in the given array", async () => {
    ...
    assert.equal(
      (await troveManager.Troves(dennis, collaterals[0].address))[3].toString(),
      "1",
    );
    assert.equal(
      (await troveManager.Troves(erin, collaterals[0].address))[3].toString(),
  ...
  it("batchLiquidateTroves: when SP > 0, triggers LQTY reward event - increases the sum G", async () => {
    ...
    // Liquidate troves
    await troveManager.batchLiquidateTroves(collaterals[0].address, [
      defaulter_1,
      defaulter_2,
    ]);
    assert.isFalse(
  ...
  it("getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption", async () => {
    ...
    const expectedICR = th
      .normalizeColl18Dedc(A_coll, collDecimals)
      .mul(price)
  ...
  it("redeemCollateral(): cancels the provided LUSD with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether", async () => {
    ...
    const dennis_ETHBalance_Before = toBN(
      await collaterals[0].balanceOf(dennis),
    );
    ...
    const alice_Trove_After = await troveManager.Troves(
      alice,
      collaterals[0].address,
    );
    const bob_Trove_After = await troveManager.Troves(
      bob,
      collaterals[0].address,
    );
    const carol_Trove_After = await troveManager.Troves(
      carol,
      collaterals[0].address,
    );
    ...
    const expectedTotalETHDrawn = th.unwrapCollNativeDec(
      redemptionAmount.div(toBN(200)),
      collDecimals,
    ); // convert redemptionAmount LUSD to ETH, at ETH:USD price 200
  ...
  it("redeemCollateral(): allowed even when minting paused", async () => {
    ...
    // pause minting
    await contracts.guardian.execute(
      lusdToken.address,
      0,
      th.getTransactionData("pauseMinting()", []),
      0,
      100_000,
    );
  ...
  it("redeemCollateral(): allowed for older versions of the protocol", async () => {
    ...
    await contracts.governance.execute(
      lusdToken.address,
      0,
      th.getTransactionData("upgradeProtocol(address,address,address)", [
  ...
  it("redeemCollateral(): works for new versions of the protocol", async () => {
    ...
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await th.openTrove(newContracts, {
      collateral: collaterals[0],
      ICR: toBN(dec(310, 16)),
      extraLUSDAmount: dec(10, 18),
      extraParams: { from: alice },
    });
    const { netDebt: B_netDebt } = await th.openTrove(newContracts, {
      collateral: collaterals[0],
      ICR: toBN(dec(290, 16)),
      extraLUSDAmount: dec(8, 18),
      extraParams: { from: bob },
    });
    const { netDebt: C_netDebt } = await th.openTrove(newContracts, {
      collateral: collaterals[0],
      ICR: toBN(dec(250, 16)),
      extraLUSDAmount: dec(10, 18),
      extraParams: { from: carol },
    ...
    // start Dennis with a high ICR
    await th.openTrove(newContracts, {
      collateral: collaterals[0],
      ICR: toBN(dec(100, 18)),
      extraLUSDAmount: redemptionAmount,
      extraParams: { from: dennis },
    ...
    const price = await newContracts.priceFeedTestnet.getPrice(
      collaterals[0].address,
    );
    assert.equal(price, dec(200, 18));
    ...
    // Find hints for redeeming 20 LUSD
    const { firstRedemptionHint, partialRedemptionHintNICR } =
      await newContracts.hintHelpers.getRedemptionHints(
        collaterals[0].address,
        redemptionAmount,
        price,
        0,
    ...
    // test case, and the list is very small, so the correct position is quickly found
    const { 0: upperPartialRedemptionHint, 1: lowerPartialRedemptionHint } =
      await newContracts.sortedTroves.findInsertPosition(
        collaterals[0].address,
        partialRedemptionHintNICR,
        dennis,
        dennis,
    ...
    // Dennis redeems 20 LUSD
    // Don't pay for gas, as it makes it easier to calculate the received Ether
    const redemptionTx = await newContracts.troveManager.redeemCollateral(
      collaterals[0].address,
      redemptionAmount,
      firstRedemptionHint,
      upperPartialRedemptionHint,
    ...
    const alice_Trove_After = await newContracts.troveManager.Troves(
      alice,
      collaterals[0].address,
    );
    const bob_Trove_After = await newContracts.troveManager.Troves(
      bob,
      collaterals[0].address,
    );
    const carol_Trove_After = await newContracts.troveManager.Troves(
      carol,
      collaterals[0].address,
    );
    ...
    const collDecimals =
      await newContracts.collateralConfig.getCollateralDecimals(
        collaterals[0].address,
      );
    const expectedTotalETHDrawn = th.unwrapCollNativeDec(
      redemptionAmount.div(toBN(200)),
  ...
  it("redeemCollateral(): with invalid first hint, trove below MCR", async () => {
    ...
    const expectedTotalETHDrawn = th
      .unwrapCollNativeDec(redemptionAmount, collDecimals)
      .div(toBN(200)); // convert redemptionAmount LUSD to ETH, at ETH:USD price 200
    const expectedReceivedETH = expectedTotalETHDrawn.sub(toBN(ETHFee));
  ...
  it("redeemCollateral(): ends the redemption sequence when the token redemption request has been filled", async () => {
    ...
    const alice_Debt = await troveManager.getTroveDebt(
      alice,
      collaterals[0].address,
    );
    const bob_Debt = await troveManager.getTroveDebt(
      bob,
      collaterals[0].address,
    );
    const carol_Debt = await troveManager.getTroveDebt(
      carol,
      collaterals[0].address,
    );
    ...
    const alice_Status = await troveManager.getTroveStatus(
      alice,
      collaterals[0].address,
    );
    const bob_Status = await troveManager.getTroveStatus(
      bob,
      collaterals[0].address,
    );
    const carol_Status = await troveManager.getTroveStatus(
      carol,
      collaterals[0].address,
    );
    ...
    const dennis_Debt = await troveManager.getTroveDebt(
      dennis,
      collaterals[0].address,
    );
    const erin_Debt = await troveManager.getTroveDebt(
      erin,
      collaterals[0].address,
    );
    ...
    const dennis_Coll = await troveManager.getTroveColl(
      dennis,
      collaterals[0].address,
    );
    const erin_Coll = await troveManager.getTroveColl(
      erin,
      collaterals[0].address,
    );
  ...
  it("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const expectedTotalETHDrawn = th
      .unwrapCollNativeDec(amount, collDecimals)
      .div(toBN(100)); // convert 100 LUSD to ETH at ETH:USD price of 100
    const expectedReceivedETH = expectedTotalETHDrawn.sub(ETHFee);
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < MCR", async () => {
    ...
    const { debt: alice_Debt_After } = await troveManager.Troves(
      alice,
      collaterals[0].address,
    );
    ...
    const { debt: bob_Debt_After } = await troveManager.Troves(
      bob,
      collaterals[0].address,
    );
  ...
  it("redeemCollateral(): finds the last Trove with ICR == MCR even if there is more than one", async () => {
    ...
    const { debt: carol_Debt_After } = await troveManager.Troves(
      carol,
      collaterals[0].address,
    );
    assert.equal(carol_Debt_After, "0");

    const { debt: dennis_Debt_After } = await troveManager.Troves(
      dennis,
      collaterals[0].address,
    );
  ...
  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    ...
    const slightlyMoreThanFee =
      await troveManager.getRedemptionFeeWithDecay(ETHDrawn);
    const tx1 = await th.redeemCollateralAndGetTxObject(
      A,
      collaterals[0].address,
      contracts,
  ...
  it("redeemCollateral(): doesn't affect the Stability Pool deposits or ETH gain of redeemed-from troves", async () => {
    ...
    const carol_ETHGain_before = (
      await stabilityPool.getDepositorCollateralGain(carol)
    )[1][0].toString();
    const dennis_ETHGain_before = (
      await stabilityPool.getDepositorCollateralGain(dennis)
    )[1][0].toString();
  ...
  it("redeemCollateral(): caller can redeem their entire LUSDToken balance", async () => {
    ...
    assert.equal(
      ...
      activePool_coll_before.sub(toBN(dec(2, collDecimals))),
    );
  ...
  it("redeemCollateral(): value of issued ETH == face value of redeemed LUSD (assuming 1 LUSD has value of $1)", async () => {
    ...
    assert.equal(
      ...
      activeETH_0.sub(
        th.unwrapCollNativeDec(
          toBN(_120_LUSD).mul(mv._1e18BN).div(price),
          collDecimals,
        ),
      ),
    );
    ...
    assert.equal(
      ...
      activeETH_1.sub(
        th.unwrapCollNativeDec(
          toBN(_373_LUSD).mul(mv._1e18BN).div(price),
          collDecimals,
        ),
      ),
    );
    ...
    assert.equal(
      ...
      activeETH_2.sub(
        th.unwrapCollNativeDec(
          toBN(_950_LUSD).mul(mv._1e18BN).div(price),
          collDecimals,
        ),
      ),
    );
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    ...
    th.assertIsApproximatelyEqual(
      (await activePool.getLUSDDebt(collaterals[0].address)).toString(),
      totalDebt,
    );
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    const A_collBefore = await troveManager.getTroveColl(
      A,
      collaterals[0].address,
    );
    const B_collBefore = await troveManager.getTroveColl(
      B,
      collaterals[0].address,
    );
    const C_collBefore = await troveManager.getTroveColl(
      C,
      collaterals[0].address,
    );
    const D_collBefore = await troveManager.getTroveColl(
      D,
      collaterals[0].address,
    );
    ...
    */
    ...
    const A_collAfter = await troveManager.getTroveColl(
      A,
      collaterals[0].address,
    );
    const B_collAfter = await troveManager.getTroveColl(
      B,
      collaterals[0].address,
    );
    const C_collAfter = await troveManager.getTroveColl(
      C,
      collaterals[0].address,
    );
    ...
    const D_collAfter = await troveManager.getTroveColl(
      D,
      collaterals[0].address,
    );
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    th.assertIsApproximatelyEqual(
      ...
      D_coll.sub(
        th.unwrapCollNativeDec(
          partialAmount.mul(mv._1e18BN).div(price),
          collDecimals,
        ),
      ),
    );
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's ETH surplus (collateral - ETH drawn) available for the trove owner to claim", async () => {
    ...
    await assertRevert(
      collSurplusPool.claimColl(A, collaterals[0].address),
      "CollSurplusPool: Caller is not Borrower Operations",
    );

    const A_GAS = th.gasUsed(
      await borrowerOperations.claimCollateral(collaterals[0].address, {
        from: A,
        gasPrice: GAS_PRICE,
      }),
    );
    ...
    th.assertIsApproximatelyEqual(
      A_balanceAfter,
      A_expectedBalance.add(
        A_coll.sub(
          th.unwrapCollNativeDec(
            A_netDebt.mul(mv._1e18BN).div(price),
            collDecimals,
          ),
        ),
      ),
    ...
    th.assertIsApproximatelyEqual(
      B_balanceAfter,
      B_expectedBalance.add(
        B_coll.sub(
          th.unwrapCollNativeDec(
            B_netDebt.mul(mv._1e18BN).div(price),
            collDecimals,
          ),
        ),
      ),
    ...
    th.assertIsApproximatelyEqual(
      C_balanceAfter,
      C_expectedBalance.add(
        C_coll.sub(
          th.unwrapCollNativeDec(
            C_netDebt.mul(mv._1e18BN).div(price),
            collDecimals,
          ),
        ),
      ),
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's ETH surplus (collateral - ETH drawn) available for the trove owner after re-opening trove", async () => {
    ...
    const A_surplus = A_collBefore.sub(
      th.unwrapCollNativeDec(
        A_netDebt.mul(mv._1e18BN).div(price),
        collDecimals,
      ),
    );
    const B_surplus = B_collBefore.sub(
      th.unwrapCollNativeDec(
        B_netDebt.mul(mv._1e18BN).div(price),
        collDecimals,
      ),
    );
    const C_surplus = C_collBefore.sub(
      th.unwrapCollNativeDec(
        C_netDebt.mul(mv._1e18BN).div(price),
        collDecimals,
      ),
  ...
  it("redeemCollateral(): reverts if fee eats up all returned collateral", async () => {
    ...
    for (let i = 0; i < 1; i++) {
      ...
      const collAmount = th.unwrapCollNativeDec(
        lusdAmount.mul(mv._1e18BN).div(price),
        collDecimals,
      );
      await mintCollateralAndApproveBorrowerOps(
        collaterals[0],
  ...
  it("getPendingLUSDDebtReward(): Returns 0 if there is no pending LUSDDebt reward", async () => {
    ...
    const carolSnapshot_L_LUSDDebt = (
      await troveManager.rewardSnapshots(carol, collaterals[0].address)
    )[1];
    assert.equal(carolSnapshot_L_LUSDDebt, 0);

    const carol_PendingLUSDDebtReward =
      await troveManager.getPendingLUSDDebtReward(
        carol,
        collaterals[0].address,
      );
  ...
  it("getPendingCollateralReward(), collaterals[0].address: Returns 0 if there is no pending ETH reward", async () => {
    ...
    // Confirm there are no pending rewards from liquidation
    const current_L_ETH = await troveManager.L_Collateral(
      collaterals[0].address,
    );
    assert.equal(current_L_ETH, 0);

    const carolSnapshot_L_ETH = (
      await troveManager.rewardSnapshots(carol, collaterals[0].address)
    )[0];
  ...
  it("computeICR(): Returns 0 if trove's coll is worth 0", async () => {
    ...
    const ICR = (
      await troveManager.computeICR(coll, debt, price, collDecimals)
    ).toString();
  ...
  it("getTroveColl(), collaterals[0].address: Returns coll", async () => {
    ...
    assert.equal(
      await troveManager.getTroveColl(A, collaterals[0].address),
      A_coll.toString(),
    );
    assert.equal(
      await troveManager.getTroveColl(B, collaterals[0].address),
      B_coll.toString(),
    );
  });
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    const A_Status = await troveManager.getTroveStatus(
      A,
      collaterals[0].address,
    );
    const B_Status = await troveManager.getTroveStatus(
      B,
      collaterals[0].address,
    );
  ...
  it("hasPendingRewards(): Returns false it trove is not active", async () => {
    assert.isFalse(
      await troveManager.hasPendingRewards(alice, collaterals[0].address),
    );
  });
});
