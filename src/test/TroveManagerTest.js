...
contract('TroveManager', async accounts => {
  ...
  it("redeemCollateral(): redemption fees keeps track of per-unit-staked error due to floor division", async () => {
    ...
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await stakingToken.approve(lqtyStaking.address, dec(100_000, 18), { from: multisig })
    await lqtyStaking.stake(dec(100_000, 18), { from: multisig })

    await openTrove({ collateral: collaterals[1], ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    await openTrove({ collateral: collaterals[1], ICR: toBN(dec(200, 16)), extraLUSDAmount: dec(100, 18), extraParams: { from: A } })
    await openTrove({ collateral: collaterals[1], ICR: toBN(dec(190, 16)), extraLUSDAmount: dec(100, 18), extraParams: { from: B } })
    await openTrove({ collateral: collaterals[1], ICR: toBN(dec(180, 16)), extraLUSDAmount: dec(100, 18), extraParams: { from: C } })
    ...
    // A redeems 10 LUSD
    await th.redeemCollateral(A, collaterals[1].address, contracts, dec(10, 18), GAS_PRICE)
    ...
    const F_ETH_Error1 = await lqtyStaking.lastF_CollateralError(collaterals[1].address);
    console.log(F_ETH_Error1.toString())
    assert.isTrue(F_ETH_Error1.gt(toBN('0')))

    // B redeems 10 LUSD
    await th.redeemCollateral(B, collaterals[1].address, contracts, dec(10, 18), GAS_PRICE)
    ...
    const F_ETH_Error2 = await lqtyStaking.lastF_CollateralError(collaterals[1].address);
