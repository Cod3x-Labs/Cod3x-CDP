...
contract("BorrowerOperations", async (accounts) => {
  ...
  const getTroveEntireDebt = async (trove, collAddress) =>
    th.getTroveEntireDebt(contracts, trove, collAddress);
  const getTroveStake = async (trove, collAddress) =>
    th.getTroveStake(contracts, trove, collAddress);
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    it("addColl(): reverts when top-up would leave trove with ICR < MCR", async () => {
      ...
      assert.isFalse(
        await troveManager.checkRecoveryMode(collaterals[0].address, price),
      );
      assert.isTrue(
        (
    ...
    it("addColl(): Increases the activePool collateral balance by correct amount", async () => {
      ...
      const activePool_RawColl_Before = await collaterals[0].balanceOf(
        activePool.address,
      );
      ...
      const activePool_RawColl_After = await collaterals[0].balanceOf(
        activePool.address,
      );
      assert.isTrue(
        activePool_Coll_After.eq(aliceColl.add(toBN(dec(1, collDecimals)))),
      );
      assert.isTrue(
        activePool_RawColl_After.eq(aliceColl.add(toBN(dec(1, collDecimals)))),
      );
    });
    ...
    it("addColl(), active Trove: Trove is in sortedList before and after", async () => {
      ...
      const aliceTroveInList_Before = await sortedTroves.contains(
        collaterals[0].address,
        alice,
      );
      ...
      const aliceTroveInList_After = await sortedTroves.contains(
        collaterals[0].address,
        alice,
      );
    ...
    it("addColl(), active Trove: updates the stake and updates the total stakes", async () => {
      ...
      assert.isTrue(
        alice_Stake_After.eq(
          alice_Stake_Before.add(toBN(dec(2, collDecimals))),
        ),
      );
      assert.isTrue(
        totalStakes_After.eq(
          totalStakes_Before.add(toBN(dec(2, collDecimals))),
        ),
      );
    });
    ...
    it("addColl(), active Trove: applies pending rewards and updates user's L_Collateral, L_LUSDDebt snapshots", async () => {
      ...
      const tx = await troveManager.liquidate(carol, collaterals[0].address, {
        from: owner,
      });
      ...
      const alice_CollateralrewardSnapshot_Before =
        alice_rewardSnapshot_Before[0];
      const alice_LUSDDebtRewardSnapshot_Before =
        alice_rewardSnapshot_Before[1];

      const bob_rewardSnapshot_Before = await troveManager.rewardSnapshots(
        bob,
        collaterals[0].address,
      );
      ...
      const alicePendingCollateralReward =
      ...
      const aliceNewColl = await getTroveEntireColl(
        alice,
        collaterals[0].address,
      );
      const aliceNewDebt = await getTroveEntireDebt(
        alice,
        collaterals[0].address,
      );
      ...
      assert.isTrue(
        aliceNewColl.eq(
          aliceCollBefore.add(alicePendingCollateralReward).add(aliceTopUp),
        ),
      );
      assert.isTrue(
      ...
      assert.isTrue(
        bobNewColl.eq(
          bobCollBefore.add(bobPendingCollateralReward).add(bobTopUp),
        ),
      );
      ...
      const alice_CollateralrewardSnapshot_After =
        alice_rewardSnapshot_After[0];
      const alice_LUSDDebtRewardSnapshot_After = alice_rewardSnapshot_After[1];

      const bob_rewardSnapshot_After = await troveManager.rewardSnapshots(
        bob,
        collaterals[0].address,
      );
      ...
      assert.isAtMost(
        th.getDifference(alice_CollateralrewardSnapshot_After, L_Collateral),
        100,
      );
      assert.isAtMost(
      ...
      assert.isAtMost(
        th.getDifference(bob_CollateralrewardSnapshot_After, L_Collateral),
        100,
      );
      assert.isAtMost(
    ...
    it("addColl(): allowed even when protocol has been upgraded", async () => {
      ...
      await th.openTrove(newContracts, {
        collateral: collaterals[0],
        value: toBN(dec(100, collDecimals)),
        extraParams: { from: alice },
      });
    });
    ...
    it("withdrawColl(): reverts when system is in Recovery Mode", async () => {
      ...
      assert.isFalse(
        await th.checkRecoveryMode(contracts, collaterals[1].address),
      );
    ...
    it("withdrawColl(): doesn’t allow a user to completely withdraw all collateral from their Trove (due to gas compensation)", async () => {
      ...
      const aliceColl = (
        await troveManager.getEntireDebtAndColl(alice, collaterals[0].address)
      )[1];
    ...
    it("withdrawColl(): reduces the Trove's collateral by the correct amount", async () => {
      ...
      const aliceCollAfter = await getTroveEntireColl(
        alice,
        collaterals[0].address,
      );
    ...
    it("withdrawColl(): reduces ActivePool ETH and raw ether by correct amount", async () => {
      ...
      assert.isTrue(
        activePool_Coll_After.eq(activePool_Coll_Before.sub(withdrawAmount)),
      );
      assert.isTrue(
        activePool_RawColl_After.eq(
          activePool_RawColl_Before.sub(withdrawAmount),
        ),
      );
    });
    ...
    it("withdrawColl(): updates the stake and updates the total stakes", async () => {
      ...
      assert.isTrue(
        alice_Stake_After.eq(
          alice_Stake_Before.sub(toBN(dec(1, collDecimals))),
        ),
      );
      assert.isTrue(
        totalStakes_After.eq(
          totalStakes_Before.sub(toBN(dec(1, collDecimals))),
        ),
      );
    });
    ...
    it("withdrawColl(): applies pending rewards and updates user's L_Collateral, L_LUSDDebt snapshots", async () => {
      ...
      await openTrove({
        collateral: collaterals[0],
        value: toBN(dec(15, collDecimals)),
        extraParams: { from: alice },
      });
      await openTrove({
        collateral: collaterals[0],
        value: toBN(dec(5, collDecimals)),
        extraParams: { from: bob },
      });
      await openTrove({
        collateral: collaterals[0],
        value: toBN(dec(1, collDecimals)),
        extraParams: { from: carol },
      });
      ...
      const aliceDebtBefore = await getTroveEntireDebt(
        alice,
        collaterals[0].address,
      );
      const bobCollBefore = await getTroveEntireColl(
        bob,
        collaterals[0].address,
      );
      const bobDebtBefore = await getTroveEntireDebt(
        bob,
        collaterals[0].address,
      );
      ...
      const pendingCollReward_A = await troveManager.getPendingCollateralReward(
        alice,
        collaterals[0].address,
      );
      ...
      const pendingCollReward_B = await troveManager.getPendingCollateralReward(
        bob,
        collaterals[0].address,
      );
      ...
      const aliceDebtAfter = await getTroveEntireDebt(
        alice,
        collaterals[0].address,
      );
      const bobCollAfter = await getTroveEntireColl(
        bob,
        collaterals[0].address,
      );
      const bobDebtAfter = await getTroveEntireDebt(
        bob,
        collaterals[0].address,
      );
      ...
      assert.isAtMost(
        th.getDifference(alice_CollrewardSnapshot_After, L_Collateral),
        100,
      );
      assert.isAtMost(
        th.getDifference(alice_LUSDDebtRewardSnapshot_After, L_LUSDDebt),
      ...
      assert.isAtMost(
        th.getDifference(bob_CollrewardSnapshot_After, L_Collateral),
        100,
      );
      assert.isAtMost(
        th.getDifference(bob_LUSDDebtRewardSnapshot_After, L_LUSDDebt),
    ...
    if (!withProxy) {
      ...
      it("withdrawLUSD(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const D_debtBefore = await getTroveEntireDebt(
          D,
          collaterals[1].address,
        );
        ...
        const newDebt = (
          await troveManager.Troves(D, collaterals[1].address)
        )[0];
    ...
    it("withdrawLUSD(): Borrowing at zero base rate changes LUSD fees-per-unit-staked", async () => {
      ...
      await stakingToken.approve(lqtyStaking.address, dec(100, 18), {
        from: A,
      });
    ...
    it("withdrawLUSD(): reverts when system is in Recovery Mode", async () => {
      ...
      assert.isTrue(txAlice.receipt.status);
      const txDennis = await borrowerOperations.withdrawLUSD(
        collaterals[1].address,
        th._100pct,
        dec(10, 18),
        dennis,
    ...
    it("withdrawLUSD(): reverts if system is in Recovery Mode", async () => {
      ...
      await priceFeed.setPrice(collaterals[0].address, "150000000000000000000");
      assert.isTrue(
        (await th.getTCR(contracts, collaterals[0].address)).lt(
          toBN(dec(165, 16)),
        ),
      );
    ...
    it("withdrawLUSD(): increases LUSD debt in ActivePool by correct amount", async () => {
      ...
      th.assertIsApproximatelyEqual(
        ...
        activePool_LUSD_Before.add(toBN(dec(1000, 18))),
      );
    });
    ...
    it("withdrawLUSD(): reverts when minting is paused", async () => {
      ...
      await contracts.governance.execute(
        lusdToken.address,
        0,
        th.getTransactionData("unpauseMinting()", []),
        0,
        100_000,
      );
      await borrowerOperations.withdrawLUSD(
    ...
    it("withdrawLUSD(): disallowed once protocol has been upgraded", async () => {
      ...
      const alice_LUSDTokenBalance_After = await lusdToken.balanceOf(alice);
      assert.isTrue(
        alice_LUSDTokenBalance_After.gt(
          alice_LUSDTokenBalance_Before.add(toBN(dec(89, 18))),
        ),
      );
      assert.isTrue(
        alice_LUSDTokenBalance_After.lt(
          alice_LUSDTokenBalance_Before.add(toBN(dec(90, 18))),
        ),
      );
    });
    ...
    it("repayLUSD(): Succeeds when it would leave trove with net debt >= minimum net debt", async () => {
      ...
      const repayTxA = await borrowerOperations.repayLUSD(
        collaterals[0].address,
        1,
        A,
        A,
      ...
      const repayTxB = await borrowerOperations.repayLUSD(
        collaterals[0].address,
        dec(19, 25),
        B,
        B,
    ...
    it("repayLUSD(): reverts when it would leave trove with net debt < minimum net debt", async () => {
      ...
      const repayTxAPromise = borrowerOperations.repayLUSD(
        collaterals[0].address,
        2,
        A,
        A,
    ...
    it("repayLUSD(): reverts when calling address does not have active trove", async () => {
      ...
      const txBob = await borrowerOperations.repayLUSD(
        collaterals[0].address,
        dec(10, 18),
        bob,
        bob,
    ...
    if (!withProxy) {
      ...
      it("adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const D_newDebt = (
          await troveManager.Troves(D, collaterals[0].address)
        )[0];
    ...
    it("adjustTrove(): debt increase that would leave ICR < 150% reverts in Recovery Mode", async () => {
      ...
      const CCR = await contracts.collateralConfig.getCollateralCCR(
        collaterals[0].address,
      );
      const collDecimals =
        await contracts.collateralConfig.getCollateralDecimals(
    ...
    it("adjustTrove(): reverts when LUSD repaid is > debt of the trove", async () => {
      ...
      const bobFee = toBN(
        await th.getEventArgByIndex(bobOpenTx, "LUSDBorrowingFeePaid", 2),
      );
      ...
      const remainingDebt = (
        await troveManager.getTroveDebt(bob, collaterals[0].address)
      ).sub(LUSD_GAS_COMPENSATION);
    ...
    it("adjustTrove(): updates borrower's debt and coll with an increase in both", async () => {
      ...
      const debtBefore = await getTroveEntireDebt(
        alice,
        collaterals[0].address,
      );
      const collBefore = await getTroveEntireColl(
        alice,
        collaterals[0].address,
      );
      ...
      th.assertIsApproximatelyEqual(
        collAfter,
        collBefore.add(toBN(dec(1, collDecimals))),
        10000,
      );
    });
    ...
    it("adjustTrove(): updates borrower's  debt and coll with coll increase, debt decrease", async () => {
      ...
      th.assertIsApproximatelyEqual(
        collAfter,
        collBefore.add(toBN(dec(5, collDecimals))),
        10000,
      );
    });
    ...
    it("adjustTrove(): updates borrower's debt and coll with coll decrease, debt increase", async () => {
      ...
      th.assertIsApproximatelyEqual(
        collAfter,
        collBefore.sub(toBN(dec(1, collDecimals))),
        10000,
      );
    });
    ...
    it("adjustTrove(): updates borrower's stake and totalStakes with a coll increase", async () => {
      ...
      assert.isTrue(
        totalStakesAfter.eq(totalStakesBefore.add(toBN(dec(1, collDecimals)))),
      );
    });
    ...
    it("adjustTrove(): updates borrower's stake and totalStakes with a coll decrease", async () => {
      ...
      assert.isTrue(
        totalStakesAfter.eq(totalStakesBefore.sub(toBN(dec(5, 11)))),
      );
    });
    ...
    it("adjustTrove(): Changes the activePool collateral balance by the requested decrease", async () => {
      ...
      assert.isTrue(
        activePool_Coll_After.eq(activePool_Coll_Before.sub(toBN(dec(1, 11)))),
      );
      assert.isTrue(
        activePool_RawColl_After.eq(
          activePool_Coll_Before.sub(toBN(dec(1, 11))),
        ),
      );
    });
    ...
    it("adjustTrove(): Changes the activePool collateral balance by the amount of collateral sent", async () => {
      ...
      const activePool_RawEther_After = await collaterals[0].balanceOf(
        activePool.address,
      );
      assert.isTrue(
        activePool_Coll_After.eq(
          activePool_Coll_Before.add(toBN(dec(1, collDecimals))),
        ),
      );
      assert.isTrue(
    ...
    it("adjustTrove(): new coll = 0 and new debt = 0 is not allowed, as gas compensation still counts toward ICR", async () => {
      ...
      const status_Before = await troveManager.getTroveStatus(
        alice,
        collaterals[0].address,
      );
      const isInSortedList_Before = await sortedTroves.contains(
        collaterals[0].address,
        alice,
      );
    ...
    it("adjustTrove(): Reverts if requested coll withdrawal is greater than trove's collateral", async () => {
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(
          ...
          th._100pct,
          0,
          aliceColl.add(toBN(dec(37, collDecimals))),
          0,
          false,
          bob,
          bob,
    ...
    it("closeTrove(): reverts when system is in Recovery Mode", async () => {
      ...
      await assertRevert(
        borrowerOperations.closeTrove(collaterals[0].address, { from: carol }),
    ...
    it("closeTrove(): reduces a Trove's debt to zero", async () => {
      ...
      const aliceDebtBefore = await getTroveEntireColl(
        alice,
        collaterals[0].address,
      );
      const dennisLUSD = await lusdToken.balanceOf(dennis);
    ...
    it("closeTrove(): sets Trove's stake to zero", async () => {
      ...
      const aliceStakeBefore = await getTroveStake(
        alice,
        collaterals[0].address,
      );
    ...
    it("closeTrove(): zero's the troves reward snapshots", async () => {
      ...
      const L_Coll_A_Snapshot = (
        await troveManager.rewardSnapshots(alice, collaterals[0].address)
      )[0];
    ...
    it("closeTrove(): reduces ActivePool collateral by correct amount", async () => {
      ...
      const dennisColl = await getTroveEntireColl(
        dennis,
        collaterals[0].address,
      );
      const aliceColl = await getTroveEntireColl(alice, collaterals[0].address);
    ...
    it("closeTrove(): reduces ActivePool debt by correct amount", async () => {
      ...
      const dennisDebt = await getTroveEntireDebt(
        dennis,
        collaterals[0].address,
      );
      const aliceDebt = await getTroveEntireDebt(alice, collaterals[0].address);
    ...
    it("closeTrove(): updates the total stakes", async () => {
      ...
      const dennisStakeBefore = await getTroveStake(
        dennis,
        collaterals[0].address,
      );
      ...
      // Check stake and total stakes get updated
      const aliceStakeAfter = await getTroveStake(
        alice,
        collaterals[0].address,
      );
      const totalStakesAfter = await troveManager.totalStakes(
    ...
    if (!withProxy) {
      ...
      it("closeTrove(): sends the correct amount of collateral to the user", async () => {
        ...
        const aliceColl = await getTroveEntireColl(
          alice,
          collaterals[0].address,
        );
        assert.isTrue(aliceColl.gt(toBN("0")));
        ...
        const alice_collBalance_After = await collaterals[0].balanceOf(alice);
        const balanceDiff = alice_collBalance_After.sub(
          alice_collBalance_Before,
        );
    ...
    it("closeTrove(): applies pending rewards", async () => {
      ...
      const liquidationTx = await troveManager.liquidate(
        carol,
        collaterals[0].address,
        { from: owner },
      );
      ...
      const defaultPool_Coll = await defaultPool.getCollateral(
        collaterals[0].address,
      );
      ...
      assert.isAtMost(
        th.getDifference(defaultPool_Coll, liquidatedColl_C),
        100,
      );
      assert.isAtMost(
      ...
      const defaultPool_Coll_afterAliceCloses = await defaultPool.getCollateral(
        collaterals[0].address,
      );
      ...
      assert.isAtMost(
        th.getDifference(
          defaultPool_Coll_afterAliceCloses,
          defaultPool_Coll.sub(pendingCollReward_A),
        ),
        1000,
      );
      assert.isAtMost(
      ...
      const defaultPool_Coll_afterBobCloses = await defaultPool.getCollateral(
        collaterals[0].address,
      );
      ...
      assert.isAtMost(
        th.getDifference(defaultPool_Coll_afterBobCloses, 0),
        100000,
      );
      assert.isAtMost(
    ...
    it("openTrove(): reverts if net debt < minimum net debt", async () => {
      ...
      const txAPromise = borrowerOperations.openTrove(
        collaterals[0].address,
        dec(100, collDecimals),
        th._100pct,
        0,
      ...
      const txCPromise = borrowerOperations.openTrove(
        ...
        dec(100, collDecimals),
        th._100pct,
        MIN_NET_DEBT.sub(toBN(dec(73, 18))),
        C,
        C,
        { from: C },
      );
    ...
    it("openTrove(): Can open a trove with ICR >= CCR when system is in Recovery Mode", async () => {
      ...
      const carol_TroveStatus = await troveManager.getTroveStatus(
        carol,
        collaterals[0].address,
      );
      assert.equal(carol_TroveStatus, 1);

      const carolICR = await troveManager.getCurrentICR(
        carol,
        collaterals[0].address,
        price,
      );
    ...
    it("openTrove(): creates a new Trove and assigns the correct collateral and debt amount", async () => {
      const debt_Before = await getTroveEntireDebt(
        alice,
        collaterals[0].address,
      );
      const coll_Before = await getTroveEntireColl(
        alice,
        collaterals[0].address,
      );
      const status_Before = await troveManager.getTroveStatus(
      ...
      const debt_After = await getTroveEntireDebt(
        alice,
        collaterals[0].address,
      );
      const coll_After = await getTroveEntireColl(
        alice,
        collaterals[0].address,
      );
      const status_After = await troveManager.getTroveStatus(
        alice,
        collaterals[0].address,
      );
    ...
    it("openTrove(): records up-to-date initial snapshots of L_Collateral and L_LUSDDebt", async () => {
      ...
      const bob_rewardSnapshot = await troveManager.rewardSnapshots(
        bob,
        collaterals[0].address,
      );
      ...
      assert.isAtMost(
        th.getDifference(bob_CollrewardSnapshot, L_Collateral),
        1000,
      );
      assert.isAtMost(
    ...
    it("openTrove(): allows a user to open a Trove, then close it, then re-open it", async () => {
      ...
      // Check Trove is active
      const alice_Trove_1 = await troveManager.Troves(
        alice,
        collaterals[0].address,
      );
      ...
      const alice_Trove_2 = await troveManager.Troves(
        alice,
        collaterals[0].address,
      );
      ...
      const alice_Trove_3 = await troveManager.Troves(
        alice,
        collaterals[0].address,
      );
    ...
    describe("getNewTCRFromTroveChange() returns the correct TCR", async () => {
      ...
      it("collChange = 0, debtChange = 0", async () => {
        ...
        assert.isFalse(
          await sortedTroves.contains(collaterals[0].address, bob),
        );
        ...
        const expectedTCR = troveColl
          ...
          .mul(price) // scale by 10^6 since WETH has 12 decimals in this test
