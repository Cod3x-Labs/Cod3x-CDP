...
contract("BorrowerWrappers", async (accounts) => {
  ...
  it("claimSPRewardsAndRecycle(): only owner can call it", async () => {
    ...
    const proxy = borrowerWrappers.getProxyFromUser(alice);
    const signature =
      "claimSPRewardsAndRecycle(address,uint256,address,address)";
  ...
  it("claimSPRewardsAndRecycle():", async () => {
    ...
    const collBalanceBefore = await collaterals[0].balanceOf(
      borrowerOperations.getProxyAddressFromUser(alice),
    );
    const troveCollBefore = await troveManager.getTroveColl(
      alice,
      collaterals[0].address,
    );
    const lusdBalanceBefore = await lusdToken.balanceOf(alice);
    const troveDebtBefore = await troveManager.getTroveDebt(
      alice,
      collaterals[0].address,
    );
    ...
    if (collDecimals.gt(toBN(18))) {
      scaledETHGain_A = scaledETHGain_A.div(
        toBN(10).pow(collDecimals.sub(toBN(18))),
      );
    } else if (collDecimals.lt(toBN(18))) {
      scaledETHGain_A = scaledETHGain_A.mul(
        toBN(10).pow(toBN(18).sub(collDecimals)),
      );
    }
    ...
    const expectedLQTYGain_A = toBN(dec(7000, 18))
      .mul(toBN(150))
      .div(toBN(2500));
    ...
    const collBalanceAfter = await collaterals[0].balanceOf(
      borrowerOperations.getProxyAddressFromUser(alice),
    );
    const troveCollAfter = await troveManager.getTroveColl(
      alice,
      collaterals[0].address,
    );
    const lusdBalanceAfter = await lusdToken.balanceOf(alice);
    const troveDebtAfter = await troveManager.getTroveDebt(
      alice,
      collaterals[0].address,
    );
  ...
  it("claimStakingGainsAndRecycle(): only owner can call it", async () => {
    ...
    // stake LQTY
    await oathToken.approve(lqtyStaking.address, dec(1850, 18), {
      from: whale,
    });
    ...
    const proxy = borrowerWrappers.getProxyFromUser(alice);
    const signature =
      "claimStakingGainsAndRecycle(address,uint256,address,address)";
    const calldata = th.getTransactionData(signature, [
      collaterals[0].address,
      th._100pct,
      alice,
  ...
  it("claimStakingGainsAndRecycle(): reverts if user has no trove", async () => {
    ...
    await openTrove({
      collateral: collaterals[0],
      extraLUSDAmount: toBN(dec(1950, 18)),
      ICR: toBN(dec(2, 18)),
      extraParams: { from: whale },
    });
    // Whale deposits 1850 LUSD in StabilityPool
    ...
    await assertRevert(
      borrowerWrappers.claimStakingGainsAndRecycle(
        collaterals[0].address,
        th._100pct,
        alice,
        alice,
  ...
  it("claimStakingGainsAndRecycle(): with only collateral gain", async () => {
    ...
    const redemptionFee =
      await troveManager.getRedemptionFeeWithDecay(redeemedAmount);
    ...
    const newBorrowingFee =
      await troveManagerOriginal.getBorrowingFeeWithDecay(netDebtChange);
  ...
  it("claimStakingGainsAndRecycle(): with both ETH and LUSD gains", async () => {
    ...
    const redemptionFee =
      await troveManager.getRedemptionFeeWithDecay(redeemedAmount);
    const scaledETHGain_A = redemptionFee
      .mul(toBN(dec(150, 18)))
      .div(toBN(dec(2000, 18)))
      .mul(mv._1e18BN)
      .div(price);
