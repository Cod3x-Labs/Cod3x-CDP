  ...
  describe("OATH Rewards", async () => {
    ...
    it("withdrawFromSP(): reward term G updates when OATH is issued", async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        dec(100, collDecimals),
        th._100pct,
        await getOpenTroveLUSDAmount(dec(9000, 18)),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 },
      );
    ...
    const getDuration = async (expectedDuration) => {
      const deploymentTime = (
        await lusdToken.getDeploymentStartTime()
      ).toNumber();
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct OATH gain. No liquidations.", async () => {
      ...
      // One week passes
      await th.fastForwardTime(
        await getDuration(timeValues.SECONDS_IN_ONE_WEEK),
        web3.currentProvider,
      );
      ...
      const expectedOATHGain_1wk = toBN(dec(14000, 18))
        .div(toBN("2"))
        .div(toBN("3"));
      ...
      assert.isAtMost(
        getDifference(A_LQTYGain_1wk, expectedOATHGain_1wk),
        2e17,
      );
      assert.isAtMost(
        getDifference(B_LQTYGain_1wk, expectedOATHGain_1wk),
        2e17,
      );
      assert.isAtMost(
        getDifference(C_LQTYGain_1wk, expectedOATHGain_1wk),
        2e17,
      );

      // Another week passes
      await th.fastForwardTime(
        timeValues.SECONDS_IN_ONE_WEEK,
        web3.currentProvider,
      );
      ...
      // Check gains are correct, error tolerance = 0.2 of a token
      assert.isAtMost(
        getDifference(A_LQTYGain_2wk, expectedOATHGain_2wk),
        2e17,
      );
      assert.isAtMost(
        getDifference(B_LQTYGain_2wk, expectedOATHGain_2wk),
        2e17,
      );
      assert.isAtMost(
        getDifference(C_LQTYGain_2wk, expectedOATHGain_2wk),
        2e17,
      );
      ...
      assert.isAtMost(
        getDifference(await oathToken.balanceOf(A), expectedOATHGain_2wk),
        2e17,
      );
      assert.isAtMost(
        getDifference(await oathToken.balanceOf(B), expectedOATHGain_2wk),
        2e17,
      );
      assert.isAtMost(
        getDifference(await oathToken.balanceOf(C), expectedOATHGain_2wk),
        2e17,
      );
    });
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct OATH gain. No liquidations.", async () => {
      ...
      assert.isAtMost(
        getDifference(A_LQTYGain_1wk, A_expectedLQTYGain_1wk),
        3e17,
      );
      assert.isAtMost(
        getDifference(B_LQTYGain_1wk, B_expectedLQTYGain_1wk),
        3e17,
      );
      assert.isAtMost(
        getDifference(C_LQTYGain_1wk, C_expectedLQTYGain_1wk),
        3e17,
      );
      ...
      const A_expectedLQTYGain_2wk = toBN(dec(14000, 18)).div(toBN("6")); // A gets 1/6 of the issuance

      const B_expectedLQTYGain_2wk = toBN(dec(14000, 18)).div(toBN("3")); // B gets 2/6 = 1/3 of the issuance

      const C_expectedLQTYGain_2wk = toBN(dec(14000, 18)).div(toBN("2")); // C gets 3/6 = 1/2 of the issuance
      ...
      // Check gains are correct, error tolerance = 0.3 of a token
      assert.isAtMost(
        getDifference(A_LQTYGain_2wk, A_expectedLQTYGain_2wk),
        3e17,
      );
      assert.isAtMost(
        getDifference(B_LQTYGain_2wk, B_expectedLQTYGain_2wk),
        3e17,
      );
      assert.isAtMost(
        getDifference(C_LQTYGain_2wk, C_expectedLQTYGain_2wk),
        3e17,
      );
      ...
      // Check OATH balances increase by correct amount
      assert.isAtMost(
        getDifference(await oathToken.balanceOf(A), A_expectedLQTYGain_2wk),
        3e17,
      );
      assert.isAtMost(
        getDifference(await oathToken.balanceOf(B), B_expectedLQTYGain_2wk),
        3e17,
      );
      assert.isAtMost(
        getDifference(await oathToken.balanceOf(C), C_expectedLQTYGain_2wk),
        3e17,
      );
    });
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct OATH gain. No liquidations.", async () => {
      ...
      assert.isAtMost(
        getDifference(A_LQTYGain_W1, A_expectedLQTYGain_W1),
        3e17,
      );
      assert.isAtMost(
        getDifference(B_LQTYGain_W1, B_expectedLQTYGain_W1),
        3e17,
      );
      assert.isAtMost(
        getDifference(C_LQTYGain_W1, C_expectedLQTYGain_W1),
        3e17,
      );
      ...
      const A_expectedTotalGain = A_expectedLQTYGain_W1.add(
        A_expectedLQTYGain_W2,
      );
      const B_expectedTotalGain = B_expectedLQTYGain_W1.add(
        B_expectedLQTYGain_W2,
      );
      const C_expectedTotalGain = C_expectedLQTYGain_W1.add(
        C_expectedLQTYGain_W2,
      );
      const D_expectedTotalGain = D_expectedLQTYGain_W2;

      // Check gains are correct, error tolerance = 0.5 of a token
      assert.isAtMost(
        getDifference(A_LQTYGain_AfterW2, A_expectedTotalGain),
        5e17,
      );
      assert.isAtMost(
        getDifference(B_LQTYGain_AfterW2, B_expectedTotalGain),
        5e17,
      );
      assert.isAtMost(
        getDifference(C_LQTYGain_AfterW2, C_expectedTotalGain),
        5e17,
      );
      assert.isAtMost(
        getDifference(D_LQTYGain_AfterW2, D_expectedTotalGain),
        5e17,
      );
      ...
      // Check OATH balances increase by correct amount
      assert.isAtMost(
        getDifference(await oathToken.balanceOf(A), A_expectedTotalGain),
        5e17,
      );
      assert.isAtMost(
        getDifference(await oathToken.balanceOf(B), B_expectedTotalGain),
        5e17,
      );
      assert.isAtMost(
        getDifference(await oathToken.balanceOf(C), C_expectedTotalGain),
        5e17,
      );
      assert.isAtMost(
        getDifference(await oathToken.balanceOf(D), D_expectedTotalGain),
        5e17,
      );
    });
    ...
    it("withdrawFromSP(): Depositor withdraws correct OATH gain after serial pool-emptying liquidations.", async () => {
      ...
      // 1 day passes
      await th.fastForwardTime(
        await getDuration(timeValues.SECONDS_IN_ONE_DAY),
        web3.currentProvider,
      );
    ...
    it("OATH issuance for a given period is not obtainable if the SP was empty during the period", async () => {
      const CIBalanceBefore = await oathToken.balanceOf(
        communityIssuanceTester.address,
      );
    ...
    it("withdrawFromSP(): Several deposits of 100 LUSD span one scale factor change. Depositors withdraw correct OATH gains", async () => {
      ...
      assert.isFalse(
        await sortedTroves.contains(collaterals[0].address, defaulter_5),
      );
      ...
      const txL6 = await troveManager.liquidate(
        defaulter_6,
        collaterals[0].address,
        { from: owner },
      );
      assert.isFalse(
        await sortedTroves.contains(collaterals[0].address, defaulter_6),
      );
