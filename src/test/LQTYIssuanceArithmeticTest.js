...
contract("Oath community issuance tests", async (accounts) => {
  ...
  it("fund(): Oath shows up at the contract address", async () => {
    ...
    const lqtyBalance = await oathToken.balanceOf(
  ...
  it("fund(): Updates last distribution time to distributionPeriod days after call", async () => {
    ...
    assert.isTrue(
      lastDistributionTwo
        .sub(lastDistributionOne)
        .eq(blockTimestampTwo.sub(blockTimestampOne)),
    );
    assert.isTrue(
      lastDistributionThree
        .sub(lastDistributionOne)
        .eq(blockTimestampThree.sub(blockTimestampOne)),
    );
    assert.isTrue(
      lastDistributionThree
        .sub(lastDistributionTwo)
        .eq(blockTimestampThree.sub(blockTimestampTwo)),
    );
    assert.isTrue(
      lastDistributionOne.sub(blockTimestampOne).eq(distributionPeriod),
    );
    assert.isTrue(
      lastDistributionTwo.sub(blockTimestampTwo).eq(distributionPeriod),
    );
    assert.isTrue(
      lastDistributionThree.sub(blockTimestampThree).eq(distributionPeriod),
    );
  });
  ...
  it("updateDistributionPeriod(): Can only be called by owner", async () => {
    await setupFunder(alice, oathToken, communityIssuanceTester, million);
    let tx = communityIssuanceTester.updateDistributionPeriod(86400, {
      from: alice,
    });
    await th.assertRevert(tx, "Ownable: caller is not the owner");
  });
  ...
  it("cannot set addresses more than once", async () => {
    let tx = communityIssuanceTester.setAddresses(
      oathToken.address,
