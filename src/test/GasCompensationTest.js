...
contract("Gas compensation tests", async (accounts) => {
  ...
  it("getCompositeDebt(): returns (debt + 10) when 0.5% of collateral > $10 in value", async () => {
    ...
    assert.isAtMost(
      th.getDifference(compositeDebt_4, "1000000010000000000000000000"),
      100000000000,
    );
  ...
  it("getCurrentICR(): Incorporates virtual debt, and returns the correct ICR for new troves", async () => {
    ...
    assert.isAtMost(
      th.getDifference(erin_ICR, "27028668628933700000"),
      10 ** 7,
    );
  ...
  it("Gas compensation from pool-offset liquidations. All collateral paid as compensation", async () => {
    ...
    const liquidatorBalance_before_A =
      await collaterals[0].balanceOf(liquidator);
    ...
    const liquidatorBalance_after_A =
      await collaterals[0].balanceOf(liquidator);

    // Check liquidator's balance increases by 0.5% of A's coll (1 ETH)
    const compensationReceived_A = liquidatorBalance_after_A.sub(
      liquidatorBalance_before_A,
    );
    const _0pt5percent_aliceColl = aliceColl.div(web3.utils.toBN("200"));
    assert.equal(
      compensationReceived_A.toString(),
      _0pt5percent_aliceColl.toString(),
    );
    ...
    const liquidatorBalance_before_B =
      await collaterals[0].balanceOf(liquidator);
    ...
    const liquidatorBalance_after_B =
      await collaterals[0].balanceOf(liquidator);

    // Check liquidator's balance increases by B's 0.5% of coll, 2 ETH
    const compensationReceived_B = liquidatorBalance_after_B.sub(
      liquidatorBalance_before_B,
    );
    const _0pt5percent_bobColl = bobColl.div(web3.utils.toBN("200"));
    assert.equal(
      compensationReceived_B.toString(),
      _0pt5percent_bobColl.toString(),
    ); // 0.5% of 2 ETH
    ...
    // Liquidate B (use 0 gas price to easily check the amount the compensation amount the liquidator receives)
    const liquidatorBalance_before_C =
      await collaterals[0].balanceOf(liquidator);
    ...
    const liquidatorBalance_after_C =
      await collaterals[0].balanceOf(liquidator);

    // Check liquidator's balance increases by C's 0.5% of coll, 3 ETH
    const compensationReceived_C = liquidatorBalance_after_C.sub(
      liquidatorBalance_before_C,
    );
    const _0pt5percent_carolColl = carolColl.div(web3.utils.toBN("200"));
    assert.equal(
      compensationReceived_C.toString(),
      _0pt5percent_carolColl.toString(),
    );
  ...
  it("gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder", async () => {
    ...
    const aliceICR = await troveManager.getCurrentICR(
      alice,
    ...
    const bobICR = await troveManager.getCurrentICR(
      bob,
      collaterals[0].address,
      price_2,
    );
  ...
  it("Gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt", async () => {
    ...
    // Liquidate A (use 0 gas price to easily check the amount the compensation amount the liquidator receives)
    const liquidationTxA = await troveManager.liquidate(
      alice,
      collaterals[0].address,
      { from: liquidator, gasPrice: GAS_PRICE },
    );
    ...
    // Liquidate B (use 0 gas price to easily check the amount the compensation amount the liquidator receives)
    const liquidationTxB = await troveManager.liquidate(
      bob,
      collaterals[0].address,
      { from: liquidator, gasPrice: GAS_PRICE },
    );
  ...
  it("liquidateTroves(): full offset.  Compensates the correct amount, and liquidates the remainder", async () => {
    ...
    const collGasCompensation =
      await troveManagerTester.getCollGasCompensation(price);
    ...
    */
    ...
    const GAS_Used_Liquidator = th.gasUsed(
      await troveManager.liquidateTroves(collaterals[0].address, 4, {
        from: liquidator,
        gasPrice: GAS_PRICE,
      }),
    );
    ...
    const compensationReceived = liquidatorBalance_after.sub(
      liquidatorBalance_before,
    );
  ...
  it("liquidateTroves(): full offset. Liquidation event emits the correct gas compensation and total liquidated coll and debt", async () => {
    ...
    */
    ...
    // Liquidate troves A-D
    const liquidationTxData = await troveManager.liquidateTroves(
      collaterals[0].address,
      4,
      { from: liquidator, gasPrice: GAS_PRICE },
    );
  ...
  it("Trove ordering: same collateral, decreasing debt. Price successively increases. Troves should maintain ordering by ICR", async () => {
    ...
    const firstColl = (
      await troveManager.Troves(_10_accounts[0], collaterals[0].address)
    )[1];
