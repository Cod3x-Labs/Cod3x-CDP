...
const contractIfForking = hre.network.config.forking
  ? contract.only
  : contract.skip;
...
contractIfForking("Leverager", async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    const { liquityCore, LQTYContracts } =
      await mainnetLocalForkUpgrade.upgrade(true);
    ...
    await collateral.approve(contracts.leverager.address, ethers.MaxUint256);
    ...
    await collateral
      .connect(whaleWallet)
      .approve(contracts.borrowerOperations.address, ethers.MaxUint256);
  });

  it("Levers up with 1 iteration", async () => {
    const ethPrice = await contracts.priceFeed.callStatic.fetchPrice(
      collateral.address,
    );
    const collAmount = ethers.utils
      .parseEther("1000")
    ...
    const collValue = (
      await contracts.troveManager.getTroveColl(deployer, collateral.address)
    )
      .mul(ethPrice)
      .div(ethers.utils.parseEther("1"));
    const debt = await contracts.troveManager.getTroveDebt(
      deployer,
      collateral.address,
    );
    th.assertIsApproximatelyEqual(
      ernBalance,
      ethers.utils.parseEther("755.4346154"),
      Number(dec(1, 12)),
    );
    th.assertIsApproximatelyEqual(
      collValue,
    ...
    th.assertIsApproximatelyEqual(
      debt,
      ethers.utils.parseEther("769.2307692"),
      Number(dec(1, 17)),
    );
  });
  ...
  it("Levers up with 2 iterations", async () => {
    ...
    assert.equal(
      await contracts.lusdToken.balanceOf(contracts.leverager.address),
      0,
    );
    ...
    th.assertIsApproximatelyEqual(
      ernBalance,
      ethers.utils.parseEther("578.0245731"),
      Number(dec(25, 18)),
    );
    th.assertIsApproximatelyEqual(
      collValue,
      ethers.utils.parseEther("1755.207985"),
      Number(dec(25, 18)),
    );
    th.assertIsApproximatelyEqual(
      debt,
      ethers.utils.parseEther("1350.159988"),
      Number(dec(25, 18)),
    );
  });
  ...
  it("Delevers and closes from 1.3 CR max leverage", async () => {
    ...
    const dollarValueRecovered = (await contracts.lusdToken.balanceOf(deployer))
      .mul(ethers.utils.parseEther("0.98686"))
      .div(ethers.utils.parseEther("1"))
      .add(
        (await collateral.balanceOf(deployer))
          .sub(initialCollBal)
          .mul(ethPrice)
          .div(ethers.utils.parseEther("1")),
      );
    th.assertIsApproximatelyEqual(
      dollarValueRecovered,
      ethers.utils.parseEther("1000"),
      Number(dec(25, 18)),
    );
  });
  ...
  it("Reverts with too many iterations", async () => {
    ...
    await assertRevert(
      contracts.leverager.leverToTargetCRWithNIterations(
        collateral.address,
