...
contract("LUSDToken", async (accounts) => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    beforeEach(async () => {
      const contracts = await deploymentHelper.deployTestCollaterals(
        await deploymentHelper.deployTesterContractsHardhat(),
      );
    ...
    it("transfer(): transferring to a blacklisted address reverts", async () => {
      ...
      await assertRevert(
        lusdTokenTester.transfer(borrowerOperations.address, 1, {
          from: alice,
        }),
      );
    });
