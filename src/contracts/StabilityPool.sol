...
import {LiquityBase, LiquityMath, IPriceFeed, IActivePool} from "./Dependencies/LiquityBase.sol";
...
contract StabilityPool is LiquityBase, Ownable, CheckContract, IStabilityPool {
    ...
    function offset(address _collateral, uint _debtToOffset, uint _collToAdd) external override {
        ...
        (uint collGainPerUnitStaked, uint LUSDLossPerUnitStaked) = _computeRewardsPerUnitStaked(
            _collateral,
            _collToAdd,
            _debtToOffset,
            totalLUSD
    ...
    function _computeRewardsPerUnitStaked(
        ...
        uint _debtToOffset,
        uint _totalLUSDDeposits
    ) internal returns (uint collGainPerUnitStaked, uint LUSDLossPerUnitStaked) {
        /*
        ...
        if (_debtToOffset == _totalLUSDDeposits) {
            ...
            lastLUSDLossError_Offset =
                (LUSDLossPerUnitStaked * _totalLUSDDeposits) -
                LUSDLossNumerator;
        }

        collGainPerUnitStaked = collNumerator / _totalLUSDDeposits;
        lastCollateralError_Offset[_collateral] =
            collNumerator -
    ...
    function _updateRewardSumAndProduct(
         ...
         */
        uint marginalCollGain = _collGainPerUnitStaked * currentP;
    ...
    function _moveOffsetCollAndDebt(
        ...
        uint sum = collAmounts[_collateral] + _collToAdd;
    ...
    function getDepositorCollateralGain(
        address _depositor
    ) public view override returns (address[] memory assets, uint[] memory amounts) {
    ...
    function _getSingularCollateralGain(
        ...
        vars.firstPortion =
            epochToScaleToSum[vars.epochSnapshot][vars.scaleSnapshot][_collateral] -
            vars.S_Snapshot;
        vars.secondPortion =
            epochToScaleToSum[vars.epochSnapshot][vars.scaleSnapshot + 1][_collateral] /
            SCALE_FACTOR;

        vars.gain =
            (_initialDeposit * (vars.firstPortion + vars.secondPortion)) /
            vars.P_Snapshot /
            DECIMAL_PRECISION;
        return vars.gain;
    }
    ...
    function _getLQTYGainFromSnapshots(
        ...
        uint LQTYGain = (initialDeposit * (firstPortion + secondPortion)) /
            P_Snapshot /
            DECIMAL_PRECISION;
    ...
    function _getCompoundedDepositFromSnapshots(
         ...
         */
        if (scaleDiff == 0) {
            compoundedDeposit = (initialDeposit * P) / snapshot_P;
        } else if (scaleDiff == 1) {
            compoundedDeposit = (initialDeposit * P) / snapshot_P / SCALE_FACTOR;
        } else {
            // if scaleDiff >= 2
            compoundedDeposit = 0;
        }
         ...
         */
        if (compoundedDeposit < initialDeposit / 1e9) {
            return 0;
        }
    ...
    function _sendCollateralGainToDepositor(address _collateral, uint _amount) internal {
        ...
        uint newCollAmount = collAmounts[_collateral] - _amount;
