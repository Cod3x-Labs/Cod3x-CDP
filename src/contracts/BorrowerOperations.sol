    ...
    IBorrowerOperations
{
    ...
    function _openTrove(
        ...
        (vars.newUpperHint, vars.newLowerHint) = sortedTroves.insert(
            _collateral,
            _borrower,
            vars.NICR,
            _upperHint,
    ...
    function _adjustTrove(
        ...
        vars.collDecimals = collateralConfig.getCollateralDecimals(
            params._collateral
        );
        ...
        contractsCache.troveManager.applyPendingRewards(
            params._borrower,
            params._collateral
        );
        ...
        if (
            ...
            !isRecoveryMode &&
            !exemptFromFee[params._borrower]
        ) {
            vars.LUSDFee = _triggerBorrowingFee(
                contractsCache.troveManager,
                contractsCache.lusdToken,
        ...
        (vars.newUpperHint, vars.newLowerHint) = sortedTroves.reInsert(
            params._borrower,
            params._collateral,
            newNICR,
            params._upperHint,
    ...
    function _closeTrove(address _borrower, address _collateral) internal {
        ...
        troveManagerCached.closeTrove(
            _borrower,
            _collateral,
            uint(TroveStatus.closedByOwner)
        );
    ...
    function _updateTroveFromAdjustment(
        ...
        uint newColl = (_isCollIncrease)
            ? _troveManager.increaseTroveColl(
                _borrower,
                _collateral,
                _collChange
            )
            : _troveManager.decreaseTroveColl(
                _borrower,
                _collateral,
                _collChange
            );
        uint newDebt = (_isDebtIncrease)
            ? _troveManager.increaseTroveDebt(
                _borrower,
                _collateral,
                _debtChange
            )
            : _troveManager.decreaseTroveDebt(
                _borrower,
                _collateral,
                _debtChange
            );
    ...
    function _activePoolAddColl(
        ...
        _activePool.pullCollateralFromBorrowerOperationsOrDefaultPool(
            _collateral,
            _amount
        );
    }
    ...
    function _requireSufficientCollateralBalanceAndAllowance(
        address _user,
        address _collateral,
        uint _collAmount
    ) internal view {
        require(
            IERC20(_collateral).balanceOf(_user) >= _collAmount,
            "BorrowerOperations: Insufficient user collateral balance"
        );
        require(
            IERC20(_collateral).allowance(_user, address(this)) >= _collAmount,
    ...
    function _requireSingularCollChange(
        ...
        require(
            _collTopUp == 0 || _collWithdrawal == 0,
    ...
    function _requireNonZeroAdjustment(
        ...
        require(
            _collTopUp != 0 || _collWithdrawal != 0 || _LUSDChange != 0,
    ...
    function _requireTroveisActive(
        ...
        require(
            status == uint(TroveStatus.active),
    ...
    function _requireCallerIsLeverager() internal view {
        require(
            msg.sender == leveragerAddress,
            "BorrowerOps: Caller is not Leverager"
        );
    }
