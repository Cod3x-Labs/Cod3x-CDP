...
contract ActivePool is Ownable, CheckContract, IActivePool {
    ...
    function setAddresses(
        ...
        address[] memory collaterals = ICollateralConfig(
            collateralConfigAddress
        ).getAllowedCollaterals();
        uint256 numCollaterals = collaterals.length;
        require(
            numCollaterals == _reaperVaults.length,
        ...
        for (uint256 i = 0; i < numCollaterals; i++) {
            ...
            require(
                IReaperVaultV2(vault).token() == collateral,
                "Vault asset must be collateral"
            );
    ...
    function setYieldGenerator(
        address _collateral,
        address _vault
    ) external override {
        _requireCallerIsOwnerOrCollateralConfig();
        require(
            yieldingAmount[_collateral] == 0,
            "All assets not withdrawn from previous vault"
        );
        require(
            IReaperVaultV2(_vault).token() == _collateral,
            "Vault asset must be collateral"
        );
        yieldGenerator[_collateral] = _vault;
    }

    function setYieldingPercentage(
        address _collateral,
        uint256 _bps
    ) external onlyOwner {
    ...
    function setYieldClaimThreshold(
        address _collateral,
        uint256 _threshold
    ) external onlyOwner {
        _requireValidCollateralAddress(_collateral);
    ...
    function manualRebalance(
        address _collateral,
        uint256 _simulatedAmountLeavingPool
    ) external onlyOwner {
        _requireValidCollateralAddress(_collateral);
    ...
    function _rebalance(
        ...
        if (vars.ownedShares != 0) {
            ...
            vars.sharesToAssets = vars.vaultBalance.mul(vars.ownedShares).div(
                vars.vaultTotalSupply
            );
        }
        ...
        vars.netAssetMovement =
            int256(vars.toDeposit) -
            int256(vars.toWithdraw) -
            int256(vars.profit);
        if (vars.netAssetMovement > 0) {
            IERC20(_collateral).safeIncreaseAllowance(
                address(vars.yieldGenerator),
                uint256(vars.netAssetMovement)
            );
            vars.yieldGenerator.deposit(uint256(vars.netAssetMovement));
        } else if (vars.netAssetMovement < 0) {
            vars.yieldGenerator.withdraw(
                uint256(-vars.netAssetMovement).mul(vars.vaultTotalSupply).div(
                    vars.vaultBalance
                )
            );
        }
        ...
        if (vars.profit != 0) {
            ...
            vars.profit = IERC20(_collateral)
                .balanceOf(address(this))
                .add(vars.yieldingAmount)
                .sub(collAmount[_collateral]);
            if (vars.profit != 0) {
