...
contract SortedTroves is Ownable, CheckContract, ISortedTroves {
    ...
    function _insert(
        ...
        data[_collateral].size = data[_collateral].size + 1;
    ...
    function _validInsertPosition(
        ...
        if (_prevId == address(0) && _nextId == address(0)) {
            ...
            return
                data[_collateral].head == _nextId &&
                _NICR >= _troveManager.getNominalICR(_nextId, _collateral);
        } else if (_nextId == address(0)) {
            // `(_prevId, null)` is a valid insert position if `_prevId` is the tail of the list
            return
                data[_collateral].tail == _prevId &&
                _NICR <= _troveManager.getNominalICR(_prevId, _collateral);
        } else {
            // `(_prevId, _nextId)` is a valid insert position if they are adjacent nodes and `_NICR` falls between the two nodes' NICRs
            return
                data[_collateral].nodes[_prevId].nextId == _nextId &&
    ...
    function _descendList(
        ...
        if (
            data[_collateral].head == _startId &&
            _NICR >= _troveManager.getNominalICR(_startId, _collateral)
        ) {
    ...
    function _ascendList(
        ...
        if (
            data[_collateral].tail == _startId &&
            _NICR <= _troveManager.getNominalICR(_startId, _collateral)
        ) {
    ...
    function _findInsertPosition(
        ...
        if (prevId != address(0)) {
            if (
                !contains(_collateral, prevId) ||
                _NICR > _troveManager.getNominalICR(prevId, _collateral)
            ) {
        ...
        if (nextId != address(0)) {
            if (
                !contains(_collateral, nextId) ||
                _NICR < _troveManager.getNominalICR(nextId, _collateral)
            ) {
