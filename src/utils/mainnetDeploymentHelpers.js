...
class MainnetDeploymentHelper {
  ...
  async loadContract(factory, address) {
    return new ethers.Contract(address, factory.interface, this.deployerWallet);
  }
  ...
  async deployLiquityCoreMainnet(
    ...
    const borrowerOperationsFactory =
      await this.getFactory("BorrowerOperations");
    ...
    // Deploy txs
    const collateralConfig = await this.loadOrDeploy(
    ...
    const redemptionHelper = await this.loadOrDeploy(
      redemptionHelperFactory,
      "redemptionHelper",
      deploymentState,
    );
    const liquidationHelper = await this.loadOrDeploy(
      liquidationHelperFactory,
      "liquidationHelper",
      deploymentState,
    );
    const leverager = await this.loadOrDeploy(
      leveragerFactory,
      "leverager",
      deploymentState,
    );
    ...
    if (this.configParams.liquityAddrs.LUSD_TOKEN) {
      lusdToken = await this.loadContract(
        lusdTokenFactory,
        this.configParams.liquityAddrs.LUSD_TOKEN,
      );
    } else {
  ...
  async isOwnerDesiredAddress(contract, desiredAddress) {
    const owner = await contract.owner();
    return (
      owner.toString().toUpperCase(owner) ==
      desiredAddress.toString().toUpperCase()
    );
  }
  ...
  async connectCoreContractsMainnet(
    ...
    (await this.isInitialized(contracts.collateralConfig)) ||
      (await this.sendAndWaitForTransaction(
        contracts.collateralConfig.initialize(
    ...
    (await this.isOwnerDesiredAddress(
      contracts.collateralConfig,
      governanceAddress,
    )) ||
      (await this.sendAndWaitForTransaction(
        contracts.collateralConfig.transferOwnership(governanceAddress, {
          gasPrice,
        }),
      ));
    ...
    (await this.isOwnerDesiredAddress(
      ...
      (await this.sendAndWaitForTransaction(
        contracts.priceFeed.transferOwnership(governanceAddress, { gasPrice }),
      ));
    ...
    (await this.isOwnershipRenounced(contracts.redemptionHelper)) ||
      (await this.sendAndWaitForTransaction(
        contracts.redemptionHelper.setAddresses(
          contracts.activePool.address,
          contracts.defaultPool.address,
          contracts.troveManager.address,
          contracts.collateralConfig.address,
    ...
    (await this.isOwnershipRenounced(contracts.liquidationHelper)) ||
      (await this.sendAndWaitForTransaction(
        contracts.liquidationHelper.setAddresses(
          contracts.activePool.address,
          contracts.defaultPool.address,
          contracts.troveManager.address,
          contracts.collateralConfig.address,
    ...
    (await this.isOwnerDesiredAddress(
      ...
      (await this.sendAndWaitForTransaction(
        contracts.borrowerOperations.transferOwnership(governanceAddress, {
          gasPrice,
        }),
      ));
    ...
    (await this.isOwnershipRenounced(contracts.leverager)) ||
      (await this.sendAndWaitForTransaction(
        contracts.leverager.setAddresses(
          contracts.borrowerOperations.address,
          contracts.collateralConfig.address,
          contracts.troveManager.address,
          contracts.activePool.address,
    ...
    (await this.isOwnershipRenounced(contracts.leverager)) ||
      (await this.sendAndWaitForTransaction(
        contracts.leverager.setExchangeSettings(
          [
  ...
  async connectLQTYContractsToCoreMainnet(
    ...
    (await this.isOwnerDesiredAddress(
      LQTYContracts.communityIssuance,
      governanceAddress,
    )) ||
      (await this.sendAndWaitForTransaction(
        LQTYContracts.communityIssuance.transferOwnership(governanceAddress, {
          gasPrice,
        }),
      ));
  }
