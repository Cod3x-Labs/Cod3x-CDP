...
class TestHelper {
  ...
  static normalizeColl18Dedc(unwrappedColl, collDecimals) {
    ...
    if (collDecimals.gt(bn_18)) {
      normalizedColl = normalizedColl.div(
        this.toBN(10).pow(collDecimals.sub(bn_18)),
      );
    } else if (collDecimals.lt(bn_18)) {
      normalizedColl = normalizedColl.mul(
        this.toBN(10).pow(bn_18.sub(collDecimals)),
      );
    }
  ...
  static unwrapCollNativeDec(normalizedColl, collDecimals) {
    ...
    if (collDecimals.gt(bn_18)) {
      unwrappedColl = unwrappedColl.mul(
        this.toBN(10).pow(collDecimals.sub(bn_18)),
      );
    } else if (collDecimals.lt(bn_18)) {
      unwrappedColl = unwrappedColl.div(
        this.toBN(10).pow(bn_18.sub(collDecimals)),
      );
    }
  ...
  static async getActualDebtFromComposite(compositeDebt, contracts) {
    const issuedDebt =
      await contracts.troveManager.getActualDebtFromComposite(compositeDebt);
    return issuedDebt;
  }
  ...
  static async getTroveEntireColl(contracts, trove, collAddress) {
    return this.toBN(
      (
        await contracts.troveManager.getEntireDebtAndColl(trove, collAddress)
      )[1],
    );
  }
  ...
  static async getEntireCollAndDebt(contracts, account, collAddress) {
    ...
    const rawColl = (
      await contracts.troveManager.Troves(account, collAddress)
    )[1];
    const rawDebt = (
      await contracts.troveManager.Troves(account, collAddress)
    )[0];
    const pendingCollateralReward =
      await contracts.troveManager.getPendingCollateralReward(
        account,
        collAddress,
      );
    const pendingLUSDDebtReward =
      await contracts.troveManager.getPendingLUSDDebtReward(
        account,
        collAddress,
      );
  ...
  static async openTrove(
    ...
    if (!ICR && !value)
      ICR = await contracts.collateralConfig.getCollateralCCR(
        collateral.address,
      );
    ...
    if (price.eq(MoneyValues._zeroBN)) {
      await contracts.priceFeedTestnet.setPrice(
        collateral.address,
        MoneyValues._100e18BN.mul(this.toBN(2)),
      );
    ...
    if (ICR) {
      ...
      collAmount = ICR.mul(totalDebt)
        .div(price)
        .div(this.toBN(10).pow(this.toBN(18).sub(collDecimals)));
    }
    ...
    if (senderCollBalance.lt(collAmount)) {
      await collateral.mint(
        extraParams.from,
        collAmount.sub(senderCollBalance),
      );
    }
    ...
    if (contracts.borrowerOperations.proxies) {
      spenderAddress =
        contracts.borrowerOperations.proxies[extraParams.from].address;
    }
  ...
  static async withdrawLUSD(
    ...
    if (ICR) {
      ...
      const targetDebt = this.normalizeColl18Dedc(coll, collDecimals)
        .mul(price)
        .div(ICR);
      assert(
        targetDebt > debt,
